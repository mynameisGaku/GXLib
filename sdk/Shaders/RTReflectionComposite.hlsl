...
float4 PSMain(FullscreenVSOutput input) : SV_Target
{
    ...
    // マテリアル roughness を先読み（ブラー半径スケーリング用）
    float4 normalSamplePre = g_Normal.SampleLevel(g_LinearClamp, uv, 0);
    float rawAlphaPre = normalSamplePre.a;
    float matRoughnessPre = rawAlphaPre - step(1.0, rawAlphaPre);
    // roughness=0→blurScale=0 (センターのみ), roughness>=0.2→blurScale=1.0 (フル25-tap)
    float blurScale = saturate(matRoughnessPre * 5.0);

    // 反射テクスチャ 深度エッジ保持ブラー (roughness連動: 鏡面ではブラーなし)
    uint rw, rh;
    ...
    {
        float2 sUV = uv + float2(offs[i]) * texel * blurScale;
