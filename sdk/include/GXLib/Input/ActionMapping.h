...
public:
    ...
    void DefineAction(const gx::String& name, const gx::Vector<InputBinding>& bindings);

    /// @brief アクションを削除する
    /// @param name アクション名
    void RemoveAction(const gx::String& name);
    ...
    const ActionState& GetAction(const gx::String& name) const;

    /// @brief アクションが押されているか
    /// @param name アクション名
    /// @return 押されていればtrue
    bool IsActionPressed(const gx::String& name) const;

    /// @brief アクションがこのフレームで押されたか
    /// @param name アクション名
    /// @return トリガーされていればtrue
    bool IsActionTriggered(const gx::String& name) const;

    /// @brief アクションがこのフレームで離されたか
    /// @param name アクション名
    /// @return リリースされていればtrue
    bool IsActionReleased(const gx::String& name) const;

    /// @brief アクションのアナログ値を取得する
    /// @param name アクション名
    /// @return アナログ値（-1.0〜1.0 or 0.0〜1.0）
    float GetActionValue(const gx::String& name) const;
    ...
    /// @param path ファイルパス
    /// @return 成功でtrue
    bool SaveToFile(const gx::String& path) const;
...
private:
    ...
    gx::HashMap<gx::String, ActionDef> m_actions;
