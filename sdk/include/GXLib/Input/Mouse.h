...
private:
    int m_x = 0, m_y = 0;           ///< 現在座標（Update時に確定）
    int m_prevX = 0, m_prevY = 0;   ///< 前フレーム座標
    int m_rawX = 0, m_rawY = 0;     ///< WM_MOUSEMOVEから受信した生の座標
    int m_wheelDelta = 0;            ///< 確定済みのホイール回転量（Update時に反映）
    int m_wheelAccum = 0;            ///< フレーム内のホイール蓄積値（複数WM_MOUSEWHEELを合算）

    gx::Array<bool, MouseButton::Count> m_currentButtons = {};   ///< 現在フレームのボタン状態
    gx::Array<bool, MouseButton::Count> m_previousButtons = {};  ///< 前フレームのボタン状態
    gx::Array<bool, MouseButton::Count> m_rawButtons = {};       ///< メッセージから受信した生のボタン状態
};
