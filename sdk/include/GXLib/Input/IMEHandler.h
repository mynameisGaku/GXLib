...
struct IMECandidate
{
    gx::Vector<gx::WString> candidates; ///< 候補文字列のリスト
...
public:
    ...
    const gx::WString& GetCompositionString() const { return m_compositionString; }

    /// @brief 確定された文字列を取得する
    /// @return 確定文字列への参照。確定直後でなければ空文字列
    const gx::WString& GetCommittedText() const { return m_committedText; }
...
private:
    ...
    gx::WString m_compositionString;
    gx::WString m_committedText;
