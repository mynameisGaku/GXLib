...
public:
    ...
    void SetListenerPosition(const Vector3& pos) { m_listenerPos = pos; }

    /// @brief レイキャストコールバックを設定する
    /// @param raycast from→to の遮蔽度 (0=遮蔽なし, 1=完全遮蔽) を返す関数
    void SetOcclusionCallback(std::function<float(const Vector3& from,
                                                   const Vector3& to)> raycast)
    {
    ...
    /// @return 遮蔽パラメータ
    OcclusionParams Calculate(const Vector3& emitterPos) const;
...
private:
    bool m_enabled = true;
    Vector3 m_listenerPos = { 0.0f, 0.0f, 0.0f };
    std::function<float(const Vector3&, const Vector3&)> m_raycastCallback;
};
