...
public:
    ...
    int LoadSound(const gx::WString& filePath);
...
private:
    ...
    struct SoundEntry
    {
        ...
        gx::WString filePath;          ///< 読み込み元パス（キャッシュ用）
    };

    gx::Vector<SoundEntry>                m_entries;       ///< ハンドル→エントリのマッピング
    gx::HashMap<gx::WString, int>  m_pathCache;     ///< パス→ハンドルの重複読み込み防止キャッシュ
