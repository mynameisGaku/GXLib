...
/// @brief ノード間で伝搬する値の型
using NodeValue = std::variant<bool, int, float, gx::String, Vector3>;
...
public:
    /// @brief コンストラクタ
    /// @param name ノード定義名
    explicit NodeDef(const gx::String& name) : m_name(name) {}
    ...
    /// @brief 入力ピン一覧を取得する
    /// @return 入力ピン記述の配列
    const gx::Vector<PinDesc>& GetInputPins() const { return m_inputPins; }

    /// @brief 出力ピン一覧を取得する
    /// @return 出力ピン記述の配列
    const gx::Vector<PinDesc>& GetOutputPins() const { return m_outputPins; }
...
private:
    gx::String m_name;                                ///< ノード定義名
    gx::Vector<PinDesc> m_inputPins;                  ///< 入力ピン定義リスト
    gx::Vector<PinDesc> m_outputPins;                 ///< 出力ピン定義リスト
...
public:
    ...
    const gx::HashMap<uint32_t, NodeValue>& GetOutputs() const { return m_outputs; }

    /// @brief トリガーされたフローピン一覧を取得する（内部用）
    /// @return フローピンインデックスの配列
    const gx::Vector<uint32_t>& GetTriggeredFlows() const { return m_triggeredFlows; }

    /// @brief トリガー済みフローをクリアする（内部用）
    void ClearTriggeredFlows() { m_triggeredFlows.clear(); }

private:
    gx::HashMap<uint32_t, NodeValue> m_inputs;  ///< 入力ピン値マップ
    gx::HashMap<uint32_t, NodeValue> m_outputs; ///< 出力ピン値マップ
    gx::Vector<uint32_t> m_triggeredFlows;            ///< トリガーされたフローピンリスト
};
...
public:
    ...
    uint32_t AddNode(const gx::String& defName);
    ...
    static const NodeDef* FindNodeDef(const gx::String& name);
...
private:
    ...
    gx::HashMap<uint32_t, NodeData> m_nodes; ///< ノードID→データマップ
    gx::Vector<Connection> m_connections;           ///< 接続リスト
    uint32_t m_nextNodeId = 0;                       ///< 次に割り当てるノードID

    /// @brief グローバルノード定義テーブルを取得する（内部用）
    /// @return ノード定義配列への参照
    static gx::Vector<std::unique_ptr<NodeDef>>& GetDefs();
};
