...
struct PluginInfo
{
    gx::String name;        ///< プラグイン名
    gx::String version;     ///< バージョン文字列
    gx::String author;      ///< 作者名
    gx::String description; ///< 説明
};
...
public:
    ...
    /// @brief プラグインの登録を解除する
    /// @return 解除成功時 true
    bool UnregisterPlugin(const gx::String& name);

    /// @brief 名前でプラグインを取得する
    /// @return プラグインポインタ（見つからない場合は nullptr）
    IPlugin* GetPlugin(const gx::String& name) const;

    /// @brief 指定名のプラグインが存在するかどうか
    bool HasPlugin(const gx::String& name) const;
    ...
    gx::Vector<PluginInfo> GetLoadedPlugins() const;
    ...
    void SetPluginEnabled(const gx::String& name, bool enabled);

    /// @brief プラグインが有効かどうか
    bool IsPluginEnabled(const gx::String& name) const;

    /// @brief プラグイン登録時コールバック
    std::function<void(const gx::String& name)> OnPluginLoaded;

    /// @brief プラグイン解除時コールバック
    std::function<void(const gx::String& name)> OnPluginUnloaded;
...
private:
    ...
    gx::Vector<PluginEntry> m_plugins;      ///< 登録プラグイン一覧
