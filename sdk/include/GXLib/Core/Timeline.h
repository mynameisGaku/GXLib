...
public:
    ...
    void SetTarget(std::function<void(const Vector3&)> target) { m_target = std::move(target); }
...
private:
    gx::Vector<Keyframe> m_keyframes;                ///< キーフレーム配列（時刻順）
    std::function<void(const Vector3&)> m_target;   ///< 値の出力先コールバック
};
...
private:
    gx::Vector<Event> m_events;   ///< イベント配列
...
private:
    gx::Vector<std::unique_ptr<TimelineTrack>> m_tracks;  ///< トラック配列
