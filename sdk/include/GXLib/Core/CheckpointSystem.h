...
struct CheckpointData
{
    ...
    gx::String name;               ///< チェックポイント名
    uint64_t timestamp = 0;         ///< 作成時刻（エポック秒）
    gx::String locationName;       ///< シーン/レベル名
    gx::String screenshotPath;     ///< スクリーンショットファイルパス
    gx::HashMap<gx::String, gx::String> metadata; ///< 任意のメタデータ
};
...
public:
    ...
    uint32_t CreateCheckpoint(const gx::String& name, const gx::String& location = "");
    ...
    gx::Vector<CheckpointData> GetAllCheckpoints() const;
    ...
    /// @param key メタデータキー
    /// @param value メタデータ値
    void SetMetadata(uint32_t checkpointId, const gx::String& key, const gx::String& value);

    /// @brief チェックポイントのメタデータを取得する
    /// @param checkpointId チェックポイントID
    /// @param key メタデータキー
    /// @return メタデータ値（未設定時は空文字列）
    gx::String GetMetadata(uint32_t checkpointId, const gx::String& key) const;

    /// @brief クイックセーブする（"QuickSave"名でチェックポイント作成）
    /// @param location シーン/レベル名（省略可）
    /// @return 作成されたチェックポイントのID
    uint32_t QuickSave(const gx::String& location = "");
    ...
    void NotifySceneChange(const gx::String& sceneName);
...
private:
    ...
    gx::Vector<CheckpointData> m_checkpoints;  ///< チェックポイント一覧
