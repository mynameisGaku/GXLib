...
inline gx::String ErrorCodeToString(ErrorCode code)
{
    char buf[16];
...
struct CrashInfo
{
    ...
    gx::String                                     message;                        ///< クラッシュメッセージ
    ErrorCode                                       errorCode = ErrorCode::None;    ///< エラーコード
    gx::String                                     probableCause;                  ///< 推測原因
    gx::Vector<gx::String>                        stackTrace;                     ///< スタックトレース
    uint32_t                                        threadId = 0;                   ///< スレッドID
    gx::String                                     moduleInfo;                     ///< モジュール情報
    gx::Vector<gx::String>                        logTail;                        ///< 直近のログ行（最大50行）
    gx::HashMap<gx::String, gx::String>    customData;                     ///< カスタムメタデータ
};

/// @brief クラッシュレポーター設定
struct CrashReporterConfig
{
    gx::String crashDumpDir     = "crashes/";   ///< ダンプファイルディレクトリ
    ...
    gx::String appName          = "GXLib";      ///< アプリケーション名
    gx::String appVersion       = "1.0.0";      ///< アプリケーションバージョン
};
...
public:
    ...
    /// @param key キー
    /// @param value 値
    void SetCustomData(const gx::String& key, const gx::String& value);

    /// @brief カスタムデータを取得する
    /// @param key キー
    /// @return 値（存在しない場合は空文字列）
    gx::String GetCustomData(const gx::String& key) const;

    /// @brief カスタムデータを削除する
    /// @param key キー
    void RemoveCustomData(const gx::String& key);

    /// @brief スタックトレースをキャプチャする
    /// @param maxFrames 最大フレーム数
    /// @return フレーム記述の配列
    gx::Vector<gx::String> CaptureStackTrace(uint32_t maxFrames = 32) const;

    /// @brief クラッシュレポートを生成する
    /// @param message クラッシュメッセージ
    /// @return クラッシュ情報
    CrashInfo CreateCrashReport(const gx::String& message);

    /// @brief クラッシュダンプをファイルに保存する
    /// @param crashInfo クラッシュ情報
    /// @param filePath ファイルパス
    /// @return 成功でtrue
    bool SaveCrashDump(const CrashInfo& crashInfo, const gx::String& filePath);

    /// @brief クラッシュダンプをファイルから読み込む
    /// @param filePath ファイルパス
    /// @return クラッシュ情報
    CrashInfo LoadCrashDump(const gx::String& filePath) const;

    /// @brief 最近のクラッシュファイル一覧を取得する
    /// @param maxCount 最大件数
    /// @return ファイルパスの配列
    gx::Vector<gx::String> GetRecentCrashes(uint32_t maxCount) const;
    ...
    /// @param crashInfo クラッシュ情報
    /// @return フォーマット済み文字列
    gx::String FormatCrashReport(const CrashInfo& crashInfo) const;
    ...
    /// @param line ログ行
    void AddLogLine(const gx::String& line);
    ...
    void ReportFatalError(ErrorCode code, const gx::String& probableCause,
                          const gx::String& message, HWND hwnd = nullptr);
