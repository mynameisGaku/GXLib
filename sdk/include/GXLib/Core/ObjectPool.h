...
private:
    ...
    gx::Vector<T>      m_storage;          ///< オブジェクト格納用配列
    gx::Vector<bool>   m_active;           ///< 各スロットの使用状態
    gx::Vector<size_t> m_freeList;         ///< 未使用スロットのインデックスリスト
    size_t              m_activeCount = 0;  ///< アクティブなオブジェクト数
    bool                m_autoExpand  = true; ///< 容量不足時の自動拡張フラグ
};

/// @brief スレッドセーフなオブジェクトプール
/// @tparam T プールするオブジェクト型（デフォルトコンストラクタ必須）
///
/// ObjectPool と同一のインターフェースを提供しつつ、すべての操作を
/// std::mutex で保護する。JobSystem 等から複数スレッドで Get()/Release() を
/// 安全に呼び出せる。
template<typename T>
...
public:
    ...
    /// @param initialCapacity 初期確保数
    /// @param autoExpand 容量不足時に自動拡張するか
    explicit ConcurrentObjectPool(size_t initialCapacity = 64, bool autoExpand = true)
        : m_autoExpand(autoExpand)
    {
        Grow(initialCapacity);
    }

    /// @brief プールからオブジェクトをスレッドセーフに取得する
    /// @return オブジェクトへのポインタ。容量不足かつ自動拡張OFFの場合nullptr
    T* Get()
    {
    ...
    /// @brief 全オブジェクトをスレッドセーフにプールに返却する
    void ReleaseAll()
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        m_freeList.clear();
    ...
    /// @brief アクティブな全オブジェクトに対して関数をスレッドセーフに実行する
    /// @param func 各オブジェクトに適用する関数
    template<typename Func>
    void ForEachActive(Func func)
    {
    ...
    /// @brief 条件を満たすオブジェクトをスレッドセーフに返却する
    /// @param predicate trueを返したオブジェクトをRelease
    template<typename Pred>
    void ReleaseIf(Pred predicate)
    {
    ...
    /// @brief アクティブなオブジェクト数をスレッドセーフに取得する
    /// @return 使用中のオブジェクト数
...
private:
    ...
    size_t              m_activeCount = 0;  ///< アクティブなオブジェクト数
    bool                m_autoExpand  = true; ///< 容量不足時の自動拡張フラグ
    mutable std::mutex  m_mutex;            ///< スレッド同期用ミューテックス
};
