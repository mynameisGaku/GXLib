...
struct ReplayFrame
{
    ...
    gx::Vector<ReplayEntityState> entities;                ///< エンティティ状態リスト
    gx::HashMap<gx::String, float> variables;       ///< カスタム変数
};

/// @brief リプレイヘッダー情報
struct ReplayHeader
{
    gx::String name;           ///< リプレイ名
...
public:
    ...
    void StartRecording(const gx::String& name = "");
    ...
    void RecordFrame(float timestamp, const gx::Vector<ReplayEntityState>& entities);

    /// @brief カスタム変数を現在のフレームに設定する
    /// @param name 変数名
    /// @param value 変数値
    void SetVariable(const gx::String& name, float value);
    ...
    const gx::Vector<ReplayFrame>& GetFrames() const;
...
private:
    ...
    gx::Vector<ReplayFrame>  m_frames;                      ///< 録画フレームデータ
    ...
    gx::HashMap<gx::String, float> m_currentVars;    ///< 現在のカスタム変数
};
...
public:
    ...
    void SetReplay(const gx::Vector<ReplayFrame>& frames, const ReplayHeader& header);
...
private:
    gx::Vector<ReplayFrame> m_frames;    ///< リプレイフレームデータ
    ReplayHeader  m_header;               ///< リプレイヘッダー
