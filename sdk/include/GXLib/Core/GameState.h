...
public:
    ...
    uint32_t FindState(const gx::String& name) const;
    ...
    const gx::String& GetCurrentStateName() const;
...
private:
    gx::Vector<GameState> m_states;       ///< 登録済みステート一覧
    gx::Vector<uint32_t>  m_stack;        ///< アクティブなステートIDスタック
    static const gx::String s_emptyName;  ///< 空文字列定数（スタック空時の戻り値）
};
