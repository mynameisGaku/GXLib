...
private:
    ...
    struct ComponentData
    {
        ...
        gx::String name;            ///< 名前（NameComponent用）
        gx::String tag;             ///< タグ（TagComponent用）
        /// @brief 軽量3次元ベクトル
    ...
    struct EntityData
    {
        ...
        gx::String name;                       ///< エンティティ名
        bool active;                            ///< アクティブフラグ
        float posX, posY, posZ;                 ///< ワールド位置
        float rotX, rotY, rotZ;                 ///< 回転（オイラー角）
        float scaleX, scaleY, scaleZ;           ///< スケール
        gx::String parentName;                 ///< 親エンティティ名（空の場合ルート）
        gx::Vector<ComponentData> components;  ///< コンポーネントデータ配列
    };

    bool m_valid = false;                       ///< スナップショットが有効かどうか
    gx::String m_sceneName;                    ///< キャプチャ時のシーン名
    gx::Vector<EntityData> m_entityData;       ///< 保存されたエンティティデータ一覧
};
