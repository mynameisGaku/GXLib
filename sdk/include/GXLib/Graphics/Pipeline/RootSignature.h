...
private:
    gx::Vector<D3D12_ROOT_PARAMETER1>      m_parameters;       ///< ルートパラメータ一覧
    gx::Vector<D3D12_STATIC_SAMPLER_DESC>  m_staticSamplers;   ///< スタティックサンプラー一覧
    /// ディスクリプタレンジの実体保持。unique_ptrで確保してポインタの安定性を保証する。
    /// vectorのリサイズで既存要素のアドレスが変わると、pDescriptorRangesが不正ポインタになるため。
    gx::Vector<std::unique_ptr<D3D12_DESCRIPTOR_RANGE1>> m_descriptorRanges;
