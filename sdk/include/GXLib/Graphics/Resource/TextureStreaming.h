...
struct TextureStreamingEntry {
    ...
    gx::WString filePath;             ///< テクスチャファイルパス
...
public:
    ...
    void RegisterTexture(int handle, const gx::WString& path, uint32_t width, uint32_t height, uint32_t mipCount);
    /// @brief テクスチャの登録を解除する
    /// @param handle テクスチャハンドル
...
private:
    ...
    gx::Vector<TextureStreamingEntry> m_textures;  ///< 登録テクスチャリスト
