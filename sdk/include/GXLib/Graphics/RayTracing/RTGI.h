...
{
    Matrix4x4 prevViewProjection; ///< 前フレームVP行列（リプロジェクション用）
    Matrix4x4 invViewProjection;  ///< 現フレーム逆VP行列
...
private:
    ...
    ShaderLightConstants m_rtLightConstants = {};       ///< ライト定数データ (HLSL cbuffer b2 と同一レイアウト)
    ...
    // テンポラル用
    Matrix4x4 m_previousVP = {};            ///< 前フレームVP行列（リプロジェクション用）
