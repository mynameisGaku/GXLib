...
{
    Vector2 sunScreenPos;    ///< UV空間の太陽位置
    float    density;          ///< 散乱密度
    float    decay;            ///< 減衰率
...
public:
    ...
    Vector2 GetLastSunScreenPos() const { return m_lastSunScreenPos; }
...
private:
    ...
    // ライトパラメータ
    Vector3 m_lightDirection = { 0.3f, -1.0f, 0.5f };  ///< ライト方向
    Vector3 m_lightColor     = { 1.0f, 0.98f, 0.95f }; ///< ライト色
    ...
    Vector2 m_lastSunScreenPos = { 0.0f, 0.0f };///< 最後に計算した太陽のスクリーンUV座標
