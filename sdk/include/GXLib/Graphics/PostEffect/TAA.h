...
struct TAAConstants
{
    Matrix4x4 invViewProjection;       ///< 現フレーム逆VP行列（非ジッター）
    Matrix4x4 previousViewProjection;  ///< 前フレームVP行列（非ジッター）
    Vector2   jitterOffset;            ///< 現フレームジッターオフセット（NDC空間）
...
public:
    ...
    Vector2 GetCurrentJitter() const;
