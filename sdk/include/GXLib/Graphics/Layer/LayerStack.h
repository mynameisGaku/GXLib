...
public:
    ...
    RenderLayer* CreateLayer(ID3D12Device* device, const gx::String& name,
                             int32_t zOrder, uint32_t w, uint32_t h);

    /// @brief 名前でレイヤーを検索する
    /// @return 見つからなければnullptr
    RenderLayer* GetLayer(const gx::String& name);

    /// @brief 名前を指定してレイヤーを削除する
    /// @return 削除できたらtrue
    bool RemoveLayer(const gx::String& name);

    /// @brief Z-order昇順にソートされたレイヤーリストを取得する
    const gx::Vector<RenderLayer*>& GetSortedLayers();
...
private:
    ...
    gx::Vector<std::unique_ptr<RenderLayer>> m_layers;
    gx::Vector<RenderLayer*> m_sortedLayers;
