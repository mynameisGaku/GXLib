...
public:
    ...
    uint32_t DeclareResource(const gx::String& name, DXGI_FORMAT format,
                              uint32_t width, uint32_t height,
                              D3D12_RESOURCE_STATES initialState = D3D12_RESOURCE_STATE_COMMON);

    /// @brief フレームグラフにバッファリソースを宣言する
    uint32_t DeclareBuffer(const gx::String& name,
                            D3D12_RESOURCE_STATES initialState = D3D12_RESOURCE_STATE_COMMON);
    ...
    uint32_t AddPass(const gx::String& name,
    ...
    const gx::Vector<uint32_t>& GetExecutionOrder() const { return m_executionOrder; }

    /// @brief 特定パス（実行インデックス指定）のコンパイル済みバリアを取得する
    const gx::Vector<BarrierEntry>& GetBarriersForPass(uint32_t executionIndex) const;
...
private:
    ...
    gx::Vector<ResourceNode> m_resources;                    ///< 宣言済みリソースノード
    gx::Vector<RenderPass> m_passes;                         ///< 登録済みレンダーパス
    gx::Vector<uint32_t> m_executionOrder;                   ///< コンパイル済み実行順序
    gx::Vector<gx::Vector<BarrierEntry>> m_passBarriers;    ///< パスごとのバリアエントリ（実行順序でインデックス）
    bool m_compiled = false;                                  ///< コンパイル済みフラグ

    static const gx::Vector<BarrierEntry> s_emptyBarriers;
};
