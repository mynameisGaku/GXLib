...
public:
    ...
    /// @brief 全ノードを取得する
    /// @return ノード配列
    const gx::Vector<BlendTreeNode>& GetNodes() const { return m_nodes; }
...
private:
    ...
    gx::Vector<BlendTreeNode> m_nodes;
    float m_param1D = 0.0f;
    float m_param2D[2] = {0, 0};

    mutable gx::Vector<TransformTRS> m_tempA;  ///< 評価用テンポラリ
    mutable gx::Vector<TransformTRS> m_tempB;
    mutable gx::Vector<TransformTRS> m_tempC;
};
