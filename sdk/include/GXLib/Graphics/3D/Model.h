...
struct MeshCPUData
{
    gx::Vector<Vertex3D_PBR>     staticVertices;   ///< 静的メッシュ頂点
    gx::Vector<Vertex3D_Skinned> skinnedVertices;  ///< スキニングメッシュ頂点
    gx::Vector<uint32_t>         indices;          ///< インデックス配列
};
...
public:
    ...
    const gx::Vector<int>& GetMaterialHandles() const { return m_materialHandles; }
    ...
    const gx::Vector<AnimationClip>& GetAnimations() const { return m_animations; }
    ...
    int FindAnimationIndex(const gx::String& name) const;
    ...
    bool ComputeAABB(Vector3& outMin, Vector3& outMax) const;
...
private:
    Mesh                       m_mesh;             ///< GPUメッシュ
    gx::Vector<int>           m_materialHandles;  ///< マテリアルハンドル配列
    std::unique_ptr<Skeleton>  m_skeleton;          ///< ボーン階層
    gx::Vector<AnimationClip> m_animations;        ///< アニメーションクリップ配列
