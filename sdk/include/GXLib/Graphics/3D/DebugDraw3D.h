...
public:
    ...
    /// @brief ワイヤーボックスを描画する
    void DrawWireBox(const Vector3& center, const Vector3& extent,
                     const Color& color);
    ...
    void DrawText3D(const Vector3& worldPos, const gx::String& text,
                    const Color& color);

    // --- 永続描画（duration秒間表示） ---

    /// @brief 永続線分を描画する
    void DrawLinePersistent(const Vector3& from, const Vector3& to,
                            const Color& color, float duration);

    /// @brief 永続ワイヤーボックスを描画する
    void DrawWireBoxPersistent(const Vector3& center, const Vector3& extent,
                               const Color& color, float duration);
    ...
    void SetViewProjection(const Matrix4x4& viewProj);
...
private:
    ...
    gx::Vector<LineEntry> m_lines;                        ///< 即時描画の線分バッファ
    gx::Vector<PersistentLine> m_persistentLines;         ///< 永続描画の線分バッファ
    gx::Vector<TextEntry> m_texts;                        ///< 3Dテキストバッファ
    Matrix4x4 m_viewProj = {};                  ///< ビュープロジェクション行列（テキスト座標変換用）
};
