...
{
    // --- 共通 (0..63) ---
    Vector4 baseColor;           //  0: albedo RGBA
    Vector3 emissiveFactor;      // 16: emissive RGB
    ...
    Vector4 shadeColor;          // 64:  1st shade color
    Vector4 shade2ndColor;       // 80:  2nd shade color
    ...
    // --- Phong (144..159) ---
    Vector3 specularColor;       // 144
    float    shininess;           // 156

    // --- Subsurface (160..191) ---
    Vector3 subsurfaceColor;     // 160
    ...
    Vector3 outlineColor;        // 212
    Vector3 highColor;           // 224: specular color
...
inline ShaderModelGPUParams ConvertFromLegacy(const struct MaterialConstants& legacy)
{
    ...
    dst.baseColor       = { legacy.albedoFactor.r, legacy.albedoFactor.g, legacy.albedoFactor.b, legacy.albedoFactor.a };
