...
struct ParticleEmitterConfig
{
    ...
    Vector4 colorStart = { 1.0f, 1.0f, 1.0f, 1.0f };  ///< 生成時の色（RGBA, x=R, y=G, z=B, w=A）
    Vector4 colorEnd   = { 1.0f, 1.0f, 1.0f, 0.0f };  ///< 寿命終了時の色（フェードアウト）
    ...
    Vector3       boxHalfExtents = { 1, 1, 1 }; ///< Boxの半サイズ

    Vector3 gravity = { 0, -9.8f, 0 }; ///< 重力加速度
...
struct Particle
{
    ...
    Vector4 color;      ///< 現在の色（RGBA, x=R, y=G, z=B, w=A）
    float    size;        ///< 現在のサイズ
...
public:
    ...
    const gx::Vector<Particle>& GetParticles() const { return m_particles; }
...
private:
    ...
    gx::Vector<Particle> m_particles;
