...
struct WaterConstants
{
    ...
    Vector4    waterColor;       ///< 水の色（RGB + 深度フォールオフ用アルファ）
...
public:
    ...
    void SetWindDirection(const Vector2& d) { m_windDirection = d; }
    /// @brief 風の方向を取得する
    const Vector2& GetWindDirection() const { return m_windDirection; }

    /// @brief 水の色を設定する（RGB + 深度フォールオフアルファ）
    void SetWaterColor(const Vector4& c) { m_waterColor = c; }
    /// @brief 水の色を取得する
    const Vector4& GetWaterColor() const { return m_waterColor; }
...
private:
    ...
    Vector2 m_windDirection = { 1.0f, 0.3f };     ///< 風の方向（XZ平面）
    Vector4 m_waterColor = { 0.0f, 0.3f, 0.5f, 0.8f };   ///< 水の色（RGB + フォールオフα）
    Vector3 m_sunDirection = { 0.3f, -1.0f, 0.5f };       ///< 太陽の方向
    Vector3 m_sunColor = { 1.0f, 0.98f, 0.95f };          ///< 太陽の色
