...
{
    Matrix4x4 viewProjection;   ///< カメラのViewProjection行列
    Vector3   topColor;         ///< 天頂の色
    float      padding1;
    Vector3   bottomColor;     ///< 地平の色
    float      padding2;
...
public:
    ...
    void SetColors(const Vector3& topColor, const Vector3& bottomColor);

    /// @brief 太陽のパラメータを設定する
    /// @param direction 太陽の方向ベクトル（正規化不要、内部で正規化しない）
    /// @param intensity HDR輝度
    void SetSun(const Vector3& direction, float intensity);

    /// @brief 天頂色を取得する
    const Vector3& GetTopColor() const { return m_topColor; }
    /// @brief 地平色を取得する
    const Vector3& GetBottomColor() const { return m_bottomColor; }
    /// @brief 太陽方向を取得する
