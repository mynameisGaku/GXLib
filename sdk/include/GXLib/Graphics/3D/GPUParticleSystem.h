...
public:
    ...
    void SetEmitterPosition(int emitterId, const Vector3& position);
    ...
    /// @brief エミッターの位置を設定
    /// @param pos ワールド座標
    void SetEmitPosition(const Vector3& pos) { m_emitPosition = pos; }
    ...
    /// @param vmin 最小速度
    /// @param vmax 最大速度
    void SetVelocityRange(const Vector3& vmin, const Vector3& vmax)
    {
        m_velocityMin = vmin;
        m_velocityMax = vmax;
    }
    ...
    void SetColorRange(const Color& startColor, const Color& endColor)
    {
...
private:
    ...
    struct GPUParticle
    {
        ...
        Color   color;       ///< 現在のカラー（RGBA）
        Color   startColor;  ///< 発生時カラー
        Color   endColor;    ///< 消滅時カラー
    }; // 96 bytes
    ...
    struct alignas(16) UpdateCB
    {
        ...
        Matrix4x4 invViewProj; ///< 逆ビュー射影行列
    }; // 160 bytes
    ...
    struct alignas(16) EmitCB
    {
        ...
        Vector3  emitPosition;   ///< エミッター位置
        Vector3  velocityMin;    ///< 速度最小値
        float    _pad0;
        Vector3  velocityMax;    ///< 速度最大値
        float    _pad1;
        float    lifeMin;         ///< 寿命最小値
        float    lifeMax;         ///< 寿命最大値
        float    sizeStart;       ///< 開始サイズ
        float    sizeEnd;         ///< 終了サイズ
        Color    colorStart;     ///< 開始カラー
        Color    colorEnd;       ///< 終了カラー
    ...
    {
        Matrix4x4 viewProj;     ///< ビュー射影行列（転置済み）
        Vector3   cameraRight;  ///< カメラ右方向ベクトル
        float     _pad0;
        Vector3   cameraUp;     ///< カメラ上方向ベクトル
        float     _pad1;
    }; // 96 bytes
    ...
    Vector3 m_emitPosition = { 0.0f, 0.0f, 0.0f };
    Vector3 m_gravity = { 0.0f, -9.8f, 0.0f };
    float    m_drag = 0.02f;

    // --- Emit parameters ---
    Vector3 m_velocityMin = { -2.0f, 5.0f, -2.0f };
    Vector3 m_velocityMax = { 2.0f, 15.0f, 2.0f };
    float   m_lifeMin = 1.0f;
    float   m_lifeMax = 3.0f;
    float   m_sizeStart = 0.2f;
    float   m_sizeEnd = 0.0f;
    Color   m_colorStart = { 1.0f, 0.8f, 0.2f, 1.0f };
    Color   m_colorEnd = { 1.0f, 0.2f, 0.0f, 0.0f };
    ...
    gx::Vector<EmitterInstance> m_emitters;
