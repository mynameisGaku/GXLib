...
public:
    ...
    const gx::Vector<SubMesh>& GetSubMeshes() const { return m_subMeshes; }

    /// @brief サブメッシュ配列を取得する（変更可）
    /// @return サブメッシュ配列への参照
    gx::Vector<SubMesh>& GetSubMeshes() { return m_subMeshes; }
    ...
    /// @return 成功した場合true
    bool CreateSmoothNormalBuffer(ID3D12Device* device, const Vector3* data, uint32_t vertexCount);
...
private:
    ...
    gx::Vector<SubMesh> m_subMeshes;  ///< サブメッシュ配列
