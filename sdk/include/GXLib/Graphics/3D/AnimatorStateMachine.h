...
struct AnimTransition
{
    ...
    gx::String triggerName;         ///< トリガー名（空なら未使用）
...
public:
    ...
    /// @param name トリガー名
    void SetTrigger(const gx::String& name);

    /// @brief floatパラメータを設定する（BlendTreeへの受け渡しに使う）
    /// @param name パラメータ名
    /// @param value 値
    void SetFloat(const gx::String& name, float value);

    /// @brief floatパラメータを取得する
    /// @param name パラメータ名
    /// @return 値（未設定なら0.0）
    float GetFloat(const gx::String& name) const;
    ...
    /// @brief 全遷移ルールを取得する
    /// @return 遷移ルール配列
    const gx::Vector<AnimTransition>& GetTransitions() const { return m_transitions; }
...
private:
    ...
    gx::Vector<AnimState> m_states;
    gx::Vector<AnimTransition> m_transitions;
    ...
    // 名前付きパラメータ
    gx::HashMap<gx::String, bool> m_triggers;   ///< トリガー（発火後に自動リセット）
    gx::HashMap<gx::String, float> m_floats;    ///< floatパラメータ

    mutable gx::Vector<TransformTRS> m_poseA;  ///< クロスフェード用テンポラリ
    mutable gx::Vector<TransformTRS> m_poseB;  ///< クロスフェード用テンポラリ
};
