...
struct TilemapLayer
{
    ...
    int width = 0;              ///< タイル列数
    int height = 0;             ///< タイル行数
    gx::Vector<int> tiles;     ///< タイル GID 配列（0 = 空タイル）
...
public:
    ...
    /// @return 成功なら true
    bool LoadFromTMX(const gx::String& path, TextureManager& texManager);
...
private:
    ...
    gx::Vector<Tileset> m_tilesets;
    gx::Vector<TilemapLayer> m_layers;
};
