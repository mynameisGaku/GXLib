...
{
    gx::String fromClip;                   ///< 遷移元クリップ名
    gx::String toClip;                     ///< 遷移先クリップ名
...
public:
    ...
    void RemoveClip(const gx::String& name);

    /// @brief クリップを取得する
    /// @param name クリップ名
    /// @return クリップへのポインタ。見つからなければ nullptr
    const SpriteAnimClip* GetClip(const gx::String& name) const;
    ...
    void Play(const gx::String& clipName);
    ...
    /// @return クリップ名への参照
    const gx::String& GetCurrentClipName() const { return m_currentClipName; }
...
private:
    ...
    gx::HashMap<gx::String, SpriteAnimClip> m_clips;    ///< クリップマップ
    gx::Vector<SpriteAnimTransition> m_transitions;             ///< 遷移ルール配列

    gx::String m_currentClipName;      ///< 現在再生中のクリップ名
    gx::String m_previousClipName;     ///< クロスフェード中の前クリップ名
