...
public:
    ...
    void DrawString(int fontHandle, float x, float y, const gx::WString& text, uint32_t color = 0xFFFFFFFF);
    ...
    /// @return 文字列の描画幅（ピクセル）
    int GetStringWidth(int fontHandle, const gx::WString& text);
    ...
    /// テキストの描画高さを計算する（改行含む）
    int GetStringHeight(int fontHandle, const gx::WString& text,
                         const TextLayoutOptions& options);
...
private:
    ...
    gx::Vector<gx::WString> BreakLines(int fontHandle, const gx::WString& text,
                                          const TextLayoutOptions& options);

    /// 単一行の描画幅を計算する（内部ヘルパー）
    float MeasureLineWidth(int fontHandle, const gx::WString& line);
