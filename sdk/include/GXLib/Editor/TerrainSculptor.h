...
struct SculptStroke
{
    ...
    gx::Vector<std::pair<float, float>> positions; ///< ブラシ適用座標リスト (x, y)
...
struct SculptAction
{
    ...
    gx::Vector<std::pair<int, int>> affectedCells;    ///< 影響を受けたセル座標 (col, row)
    gx::Vector<float> oldValues;                      ///< 変更前の値
    gx::Vector<float> newValues;                      ///< 変更後の値
};
...
public:
    ...
    gx::Vector<std::pair<std::pair<int, int>, float>> GetBrushMask(float centerX, float centerY) const;
    ...
    static gx::Vector<Vector3> ComputeNormals(const float* heightmap, int width, int height);
...
private:
    ...
    gx::Vector<SculptAction> m_history;               ///< 操作履歴（Undo用）
};
