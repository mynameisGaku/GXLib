...
public:
    ...
    GizmoAxis HitTest(const Vector3& rayOrigin,
                       const Vector3& rayDir,
                       const Transform3D& transform) const;

    /// @brief 指定した軸でドラッグ操作を開始する
    void BeginDrag(GizmoAxis axis, const Vector3& rayOrigin,
                   const Vector3& rayDir);

    /// @brief 進行中のドラッグを更新し、結果をtransformに適用する
    void UpdateDrag(const Vector3& rayOrigin,
                    const Vector3& rayDir,
...
private:
    /// レイと軸線分の最短距離を計算する
    float RayAxisDistance(const Vector3& rayOrigin,
                          const Vector3& rayDir,
                          const Vector3& axisOrigin,
                          const Vector3& axisDir) const;

    /// レイと平面の交点を求める
    Vector3 RayPlaneIntersect(const Vector3& rayOrigin,
                                          const Vector3& rayDir,
                                          const Vector3& planePoint,
                                          const Vector3& planeNormal) const;

    /// レイをドラッグ軸に投影し、軸上のパラメトリック位置を返す
    float ProjectOntoAxis(const Vector3& rayOrigin,
                           const Vector3& rayDir,
                           const Vector3& axisOrigin,
                           const Vector3& axisDir) const;
    ...
    Vector3 m_dragStartPos       = {};   ///< ドラッグ開始時のレイ-軸交点
    Vector3 m_dragStartObjectPos = {};   ///< ドラッグ開始時のオブジェクト位置
    Vector3 m_dragStartScale     = {};   ///< ドラッグ開始時のオブジェクトスケール
