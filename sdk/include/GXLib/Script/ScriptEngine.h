...
public:
    ...
    /// @param path ファイルパス
    /// @return 成功なら true
    bool ExecuteFile(const gx::String& path);

    /// @brief Lua コード文字列を実行する
    /// @param code Lua コード
    /// @return 成功なら true
    bool ExecuteString(const gx::String& code);

    /// @brief Lua のグローバル関数を呼び出す（引数なし）
    /// @param funcName 関数名
    /// @return 成功なら true
    bool CallFunction(const gx::String& funcName);

    /// @brief Lua のグローバル関数を呼び出す（float引数1つ）
    /// @param funcName 関数名
    /// @param arg1 引数
    /// @return 成功なら true
    bool CallFunction(const gx::String& funcName, float arg1);

    /// @brief グローバル変数を設定する（float）
    /// @param name 変数名
    /// @param value 値
    void SetGlobal(const gx::String& name, float value);

    /// @brief グローバル変数を設定する（int）
    /// @param name 変数名
    /// @param value 値
    void SetGlobal(const gx::String& name, int value);

    /// @brief グローバル変数を設定する（string）
    /// @param name 変数名
    /// @param value 値
    void SetGlobal(const gx::String& name, const gx::String& value);

    /// @brief グローバル変数を取得する（float）
    /// @param name 変数名
    /// @param defaultValue 変数が存在しない場合の既定値
    /// @return 変数の値
    float GetGlobalFloat(const gx::String& name, float defaultValue = 0.0f) const;

    /// @brief グローバル変数を取得する（int）
    /// @param name 変数名
    /// @param defaultValue 変数が存在しない場合の既定値
    /// @return 変数の値
    int GetGlobalInt(const gx::String& name, int defaultValue = 0) const;
...
private:
    ...
    gx::String m_lastError;      ///< 最後のエラーメッセージ
};
