...
{
public:
    using Value = std::variant<bool, int, float, gx::String, Vector3>;

    /// @brief キーに値を設定する
    /// @param key キー文字列
    /// @param value 格納する値
    void Set(const gx::String& key, const Value& value) { m_data[key] = value; }
    ...
    template<typename T>
    T Get(const gx::String& key, const T& defaultValue = T{}) const
    {
    ...
    /// @param key キー文字列
    /// @return キーが存在すればtrue
    bool Has(const gx::String& key) const { return m_data.count(key) > 0; }

    /// @brief キーを削除する
    /// @param key 削除するキー文字列
    void Remove(const gx::String& key) { m_data.erase(key); }

    /// @brief 全データをクリアする
    void Clear() { m_data.clear(); }

private:
    gx::HashMap<gx::String, Value> m_data;  ///< キーバリューデータストア
};
...
private:
    gx::Vector<std::shared_ptr<BTNode>> m_children;  ///< 子ノードリスト
