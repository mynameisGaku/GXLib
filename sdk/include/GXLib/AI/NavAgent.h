...
public:
    ...
    const gx::Vector<Vector3>& GetPath() const { return m_path; }
    ...
    void UpdateWithNeighbors(float deltaTime, const gx::Vector<NavAgent*>& neighbors);

    /// @brief Get the last computed velocity (from most recent Update)
    Vector3 GetLastVelocity() const { return m_lastVelocity; }
...
private:
    ...
    gx::Vector<Vector3> m_path;
    int      m_currentPathIndex = 0;
    Vector3 m_position = { 0.0f, 0.0f, 0.0f };
    Vector3 m_lastVelocity = { 0.0f, 0.0f, 0.0f };
    float    m_yaw      = 0.0f;
