...
{
    gx::WString filePath;            ///< 読み取り対象ファイルパス
...
public:
    ...
    /// @return 成功した場合true
    bool OpenFile(const gx::WString& path);

    /// @brief 以前に開いたファイルを閉じる
    void CloseFile(const gx::WString& path);
...
private:
    ...
    struct RequestInfo
    {
        ...
        gx::WString filePath;        ///< 対象ファイルパス
        uint64_t offset;              ///< ファイル内オフセット
        uint32_t size;                ///< 読み取りサイズ
        void* destination;            ///< 読み取り先バッファ
    };
    gx::Vector<RequestInfo> m_requests;               ///< アクティブなリクエスト一覧
    gx::HashMap<gx::WString, bool> m_openFiles; ///< 開いているファイルの管理
};
