...
public:
    ...
    bool Connect(const gx::String& url);
    ...
    bool Send(const gx::String& message);
    ...
    std::function<void(const gx::String&)> onMessage;
    /// @brief バイナリメッセージ受信時のコールバック
    std::function<void(const void*, size_t)> onBinaryMessage;
    /// @brief 接続終了時のコールバック
    std::function<void()> onClose;
    /// @brief エラー発生時のコールバック
    std::function<void(const gx::String&)> onError;
...
private:
    ...
    gx::Vector<QueuedMessage> m_messageQueue;
