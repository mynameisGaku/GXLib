...
{
    gx::Vector<uint8_t> data;       ///< シリアライズされたゲーム状態
...
struct RollbackCallbacks
{
    ...
    std::function<void(const gx::Vector<PlayerInput>&)> advanceFrame;
    /// チェックサムを計算する
...
private:
    ...
    gx::Array<gx::Vector<PlayerInput>, 4> m_inputHistory;

    /// @brief プレイヤーごとの確定済み入力フラグ（リングバッファ）
    gx::Array<gx::Vector<bool>, 4> m_inputConfirmed;

    /// @brief ゲーム状態履歴（リングバッファ）
    gx::Vector<GameStateSnapshot> m_stateHistory;
