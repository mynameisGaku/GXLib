...
public:
    ...
    /// @param position ルートの初期位置
    /// @return 構築に成功した場合true
    bool BuildCustom(PhysicsWorld3D* world, const gx::Vector<RagdollBone>& bones,
    ...
    /// @param world 物理ワールド
    /// @return 各ボーンの4x4変換行列
    gx::Vector<Matrix4x4> GetBoneTransforms(PhysicsWorld3D* world) const;

    /// @brief ボーン配列を取得する
    const gx::Vector<RagdollBone>& GetBones() const { return m_bones; }

    /// @brief ジョイント配列を取得する
    const gx::Vector<RagdollJoint>& GetJoints() const { return m_joints; }
...
private:
    ...
    gx::Vector<RagdollBone> m_bones;
    gx::Vector<RagdollJoint> m_joints;
    bool m_built = false;
};
