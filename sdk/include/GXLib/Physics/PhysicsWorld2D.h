...
public:
    ...
    void QueryAABB(const AABB2D& area, gx::Vector<RigidBody2D*>& results);
...
private:
    gx::Vector<std::unique_ptr<RigidBody2D>> m_bodies;
    Vector2 m_gravity = { 0.0f, -9.81f };

    void BroadPhase(gx::Vector<std::pair<RigidBody2D*, RigidBody2D*>>& pairs);
