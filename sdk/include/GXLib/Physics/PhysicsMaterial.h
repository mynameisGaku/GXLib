...
public:
    ...
    /// @param params パラメータ
    /// @return マテリアルID
    uint32_t Register(const gx::String& name, const PhysicsMaterialParams& params);

    /// @brief 名前でマテリアルを検索する
    /// @param name マテリアル名
    /// @return 見つかった場合はパラメータへのポインタ、なければ nullptr
    const PhysicsMaterialParams* Find(const gx::String& name) const;
...
private:
    ...
    gx::HashMap<gx::String, uint32_t> m_nameIndex;
};
