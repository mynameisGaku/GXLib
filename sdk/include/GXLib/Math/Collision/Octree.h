...
public:
    ...
    void Query(const AABB3D& area, gx::Vector<T>& results) const
    {
    ...
    /// @param area 検索範囲の球
    /// @param results 見つかったオブジェクトの出力先
    void Query(const Sphere& area, gx::Vector<T>& results) const
    {
        AABB3D sphereBounds(
    ...
    /// @param results 見つかったオブジェクトの出力先
    void Query(const Frustum& frustum, gx::Vector<T>& results) const
    {
    ...
    void GetPotentialPairs(gx::Vector<std::pair<T, T>>& pairs) const
    {
        gx::Vector<std::pair<T, AABB3D>> ancestors;
...
private:
    struct Node {
        AABB3D bounds;
        gx::Vector<std::pair<T, AABB3D>> objects;
    ...
    void QueryNode(const Node& node, const AABB3D& area, gx::Vector<T>& results) const
    {
    ...
    void QueryNodeSphere(const Node& node, const Sphere& sphere, const AABB3D& sphereBounds, gx::Vector<T>& results) const
    {
    ...
    void QueryNodeFrustum(const Node& node, const Frustum& frustum, gx::Vector<T>& results) const
    {
    ...
    void GetPairsFromNode(const Node& node, gx::Vector<std::pair<T, AABB3D>>& ancestors,
                          gx::Vector<std::pair<T, T>>& pairs) const
    {
        for (const auto& [obj, bnd] : node.objects)
        {
