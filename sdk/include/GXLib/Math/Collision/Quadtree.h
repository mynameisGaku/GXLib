...
public:
    ...
    void Query(const AABB2D& area, gx::Vector<T>& results) const
    {
    ...
    /// @param area 検索範囲の円
    /// @param results 見つかったオブジェクトの出力先
    void Query(const Circle& area, gx::Vector<T>& results) const
    {
        AABB2D circleBounds(
    ...
    {
        gx::Vector<std::pair<T, AABB2D>> ancestors;
...
private:
    struct Node {
        AABB2D bounds;
        gx::Vector<std::pair<T, AABB2D>> objects;
    ...
    void QueryNode(const Node& node, const AABB2D& area, gx::Vector<T>& results) const
    {
    ...
    void QueryNodeCircle(const Node& node, const Circle& circle, const AABB2D& circleBounds, gx::Vector<T>& results) const
    {
    ...
    void GetPairsFromNode(const Node& node, gx::Vector<std::pair<T, AABB2D>>& ancestors,
