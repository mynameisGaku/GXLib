...
struct UIRectVertex
{
    ...
    Vector2 localUV;   ///< ローカルUV（SDF計算用、0~1）
};
...
struct UIRectConstants
{
    ...
    Vector2    rectSize;           ///< 矩形サイズ（ピクセル）
    float      cornerRadius;       ///< 角丸半径（ピクセル）
    float      borderWidth;        ///< ボーダー幅（ピクセル）
    Color      fillColor;          ///< 塗りつぶし色（RGBA）
    Color      borderColor;        ///< ボーダー色（RGBA）
    Vector2    shadowOffset;       ///< 影オフセット（ピクセル）
    float      shadowBlur;         ///< 影ぼかし半径（ピクセル）
    float      shadowAlpha;        ///< 影の不透明度
    float      opacity;            ///< 全体の不透明度
    float      _pad[3];            ///< パディング
    Color      gradientColor;      ///< グラデーション終了色（RGBA）
    Vector2    gradientDir;        ///< グラデーション方向ベクトル
    float      gradientEnabled;    ///< グラデーション有効フラグ（0/1）
    float      _pad2;              ///< パディング
    Vector2    effectCenter;       ///< エフェクト中心（UV, 0~1）
...
public:
    ...
    /// @param color テキスト色
    /// @param opacity 不透明度
    void DrawText(float x, float y, int fontHandle, const gx::WString& text,
    ...
    /// @param text 計測するテキスト
    /// @return テキスト幅（ピクセル）
    int GetTextWidth(int fontHandle, const gx::WString& text);
...
private:
    ...
    Matrix4x4 m_projectionMatrix;                        ///< 2D正射影行列
    ...
    // Scissor
    gx::Vector<ScissorRect> m_scissorStack;             ///< シザー矩形スタック
    ScissorRect m_fullScreen;                            ///< フルスクリーンシザー矩形
    bool m_spriteBatchActive = false;                    ///< SpriteBatchが描画中か

    // Deferred
    gx::Vector<std::function<void()>> m_deferredDraws;  ///< 遅延描画コールバックキュー

    // Transform/Opacity stacks
    gx::Vector<Transform2D> m_transformStack;           ///< アフィン変換スタック
    gx::Vector<float> m_opacityStack;                   ///< 不透明度スタック
};
