...
public:
    ...
    /// @return 子ウィジェットの配列
    const gx::Vector<std::unique_ptr<Widget>>& GetChildren() const { return m_children; }

    /// @brief 子ウィジェット一覧を取得する
    /// @return 子ウィジェットの配列
    gx::Vector<std::unique_ptr<Widget>>& GetChildren() { return m_children; }

    /// @brief IDでウィジェットをツリー全体から再帰検索する
    /// @param id 検索するID文字列
    /// @return 見つかったウィジェット。なければ nullptr
    Widget* FindById(const gx::String& id);

    // --- プロパティ ---
    gx::String id;              ///< ウィジェットの一意識別子（FindByIdで使う）
    gx::String className;       ///< CSSクラス名（StyleSheetのセレクタで使う）
    ...
    std::function<void(const gx::String&)> onValueChanged;
...
protected:
    ...
    gx::Vector<std::unique_ptr<Widget>> m_children;       ///< 子ウィジェット配列
