...
struct ClothDesc
{
    ...
    DirectX::XMFLOAT3 gravity = { 0.0f, -9.8f, 0.0f }; ///< 重力
    std::vector<uint32_t> pinnedVertices; ///< 初期固定頂点インデックス
};
...
public:
    ...
    /// @param center 球の中心座標
    /// @param radius 球の半径
    void AddSphereCollider(const DirectX::XMFLOAT3& center, float radius);
    ...
    /// @brief 頂点法線を取得する
    const std::vector<DirectX::XMFLOAT3>& GetNormals() const { return m_normals; }
...
private:
    ...
    DirectX::XMFLOAT3 m_externalForce = { 0.0f, 0.0f, 0.0f };

    std::vector<DirectX::XMFLOAT3> m_positions;
    std::vector<DirectX::XMFLOAT3> m_prevPositions;
