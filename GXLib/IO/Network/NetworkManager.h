...
/// @brief RPCハンドラ型
using RPCHandler = std::function<void(ClientId sender, const std::vector<uint8_t>& data)>;
...
public:
    ...
    void SendTo(ClientId client, const std::vector<uint8_t>& data);
    ...
    /// @param name RPC名
    /// @param args 引数データ
    void CallRPC(const std::string& name, const std::vector<uint8_t>& args);

    /// @brief 特定クライアントにRPCを呼び出す（サーバーのみ）
    /// @param target 対象クライアントID
    /// @param name RPC名
    /// @param args 引数データ
    void CallRPCOn(ClientId target, const std::string& name, const std::vector<uint8_t>& args);
...
private:
    ...
    void SendPacket(const std::string& host, uint16_t port,
                    PacketType type, const void* data, uint16_t size);
    void ProcessPacket(const std::string& fromHost, uint16_t fromPort,
    ...
    // RPC
    std::unordered_map<uint32_t, RPCHandler> m_rpcHandlers;
    std::unordered_map<std::string, uint32_t> m_rpcNameToHash;
