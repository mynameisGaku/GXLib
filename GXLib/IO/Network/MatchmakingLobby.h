...
struct PlayerInfo
{
    ...
    std::unordered_map<std::string, std::string> customData;  ///< カスタムデータ
};
...
struct LobbyConfig
{
    ...
    std::string lobbyName;           ///< ロビー名
    bool isPrivate = false;          ///< プライベートロビーか
    std::string password;            ///< パスワード（空=パスワードなし）
    bool autoStart = true;           ///< 全員レディで自動開始するか
    int countdownSeconds = 5;        ///< 開始カウントダウン（秒）
    std::string gameMode = "default"; ///< ゲームモード
    std::string region = "auto";     ///< 地域（"auto"で自動選択）
...
struct MatchFilter
{
    std::string gameMode;              ///< ゲームモードフィルタ
    ...
    std::string region;                ///< 地域フィルタ
    std::vector<std::string> tags;     ///< タグフィルタ
};
...
struct MatchResult
{
    ...
    std::string serverAddress;     ///< 接続先サーバーアドレス
...
public:
    ...
    bool JoinByCode(const std::string& inviteCode);
    ...
    std::vector<PlayerInfo> FindPlayers() const;
    ...
    /// @brief チャット履歴を取得する
    std::vector<std::pair<std::string, std::string>> GetChatHistory() const { return m_chatHistory; }
...
private:
    ...
    std::vector<std::pair<std::string, std::string>> m_chatHistory;  ///< チャット履歴（名前, メッセージ）
    std::unordered_map<std::string, uint64_t> m_inviteCodes;         ///< 招待コード → ロビーIDマップ
};
