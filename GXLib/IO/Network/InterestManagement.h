...
public:
    ...
    std::vector<uint32_t> QueryRelevantEntities(uint32_t observerEntityId) const;
...
private:
    ...
    std::unordered_map<uint32_t, InterestEntity> m_entities;  ///< エンティティID→位置情報マップ

    /// グリッド: cellId → エンティティIDセット
    std::unordered_map<int64_t, std::unordered_set<uint32_t>> m_grid;
};
