...
public:
    ...
    /// @return バイナリデータ
    std::vector<uint8_t> SerializeSnapshot(const std::vector<SnapshotEntry>& entries) const;

    /// @brief スナップショットをデシリアライズする
    /// @param data バイナリデータ
    /// @return エントリのリスト
    std::vector<SnapshotEntry> DeserializeSnapshot(const std::vector<uint8_t>& data) const;

    /// @brief デルタスナップショットをシリアライズする
    /// @param current 現在のスナップショット
    /// @param previous 前回のスナップショット
    /// @return デルタバイナリデータ
    std::vector<uint8_t> SerializeDeltaSnapshot(const std::vector<SnapshotEntry>& current,
    ...
    /// @return 更新済みスナップショット
    std::vector<SnapshotEntry> ApplyDeltaSnapshot(const std::vector<SnapshotEntry>& base,
...
private:
    ...
    std::vector<RegisteredEntity> m_entities;
    std::vector<InterpolationBuffer> m_interpolationBuffers;
    std::vector<SnapshotEntry> m_lastSnapshot;
