...
struct ReceivedPacket
{
    ...
    gx::Vector<uint8_t> data;   ///< ペイロードデータ
    gx::String fromHost;        ///< 送信元ホスト
...
public:
    ...
    /// @param data ペイロードデータ
    /// @param size データサイズ
    void SendUnreliable(const gx::String& host, uint16_t port,
                        uint16_t type, const void* data, uint32_t size);
    ...
    /// @param port 送信元ポート
    /// @return 処理済みパケットのリスト
    gx::Vector<ReceivedPacket> ProcessIncoming(const uint8_t* rawData, uint32_t size,
...
private:
    ...
    /// @brief 生パケットを送信する
    void SendRaw(const gx::String& host, uint16_t port,
    ...
    gx::Vector<PendingPacket> m_pendingPackets;    ///< ACK待ちパケット
    gx::Vector<uint16_t> m_receivedSequences;      ///< 受信済みシーケンスリスト
