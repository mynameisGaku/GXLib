...
struct AccessibilityState
{
    ...
    std::string description;     ///< 説明テキスト（スクリーンリーダー用）
};
...
struct AccessibleElement
{
    ...
    std::string label;                                 ///< アクセシブルラベル
    std::string hint;                                  ///< 補助テキスト（ヒント）
    AccessibilityState state;                          ///< 現在の状態
    uint32_t parentId = 0;                             ///< 親要素ID
    std::vector<uint32_t> childIds;                    ///< 子要素IDリスト
    int tabOrder = 0;                                  ///< タブ順序
};
...
public:
    ...
    void Announce(const std::string& text, AnnouncementPriority priority = AnnouncementPriority::Medium);

    /// @brief Get all pending announcements
    /// @return Vector of screen reader events
    std::vector<ScreenReaderEvent> GetPendingAnnouncements() const;
    ...
    std::vector<uint32_t> GetTabOrder() const;
    ...
    std::vector<AccessibleElement> GetAccessibleTree() const;

    /// @brief Find elements by label text
    /// @param label The label to search for
    /// @return Vector of matching element IDs
    std::vector<uint32_t> FindByLabel(const std::string& label) const;
...
private:
    ...
    std::unordered_map<uint32_t, AccessibleElement> m_elements;      ///< 登録済み要素マップ
    uint32_t m_nextElementId = 1;                                    ///< 次に割り当てる要素ID
    uint32_t m_focusedElementId = 0;                                 ///< フォーカス中の要素ID
    std::vector<ScreenReaderEvent> m_pendingAnnouncements;           ///< 未処理のアナウンスキュー
