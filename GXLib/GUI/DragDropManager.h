...
struct DragPayload
{
    std::string type;                ///< ペイロードの型識別子（例: "item", "color"）
    std::vector<uint8_t> data;       ///< バイナリデータ本体
    std::string text;                ///< テキスト表現（デバッグ/簡易用途）
};
...
struct DropTarget
{
    std::string acceptType;   ///< 受け入れるペイロード型（""なら全て受け入れ）
...
public:
    ...
    void RegisterDragSource(Widget* widget, const std::string& payloadType);
...
private:
    ...
    std::unordered_map<Widget*, std::string> m_sources;    ///< ウィジェット→ペイロード型マップ
    std::unordered_map<Widget*, DropTarget> m_targets;     ///< ウィジェット→ドロップターゲット情報マップ
