...
struct AudioSnapshotData
{
    ...
    std::vector<BusSnapshot> buses;  ///< バス設定リスト
...
public:
    ...
    void UnregisterSnapshot(const std::string& name);

    /// @brief スナップショットを取得する
    /// @param name スナップショット名
    /// @return スナップショットデータへのポインタ（見つからなければnullptr）
    const AudioSnapshotData* GetSnapshot(const std::string& name) const;
    ...
    void BlendToSnapshot(const std::string& name, float transitionTime);
    ...
    std::string GetCurrentSnapshotName() const;
...
private:
    std::unordered_map<std::string, AudioSnapshotData> m_snapshots;
