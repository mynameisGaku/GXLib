...
private:
    ...
    // --- データ ---
    std::vector<EntityRecord> m_entities;   ///< EntityIDでインデックス（スロット0は未使用）
    std::vector<Archetype> m_archetypes;   ///< 全アーキタイプのリスト
    std::vector<EntityID> m_freeList;      ///< 再利用可能なEntityIDのフリーリスト
    uint32_t m_aliveCount = 0;             ///< 現在生存中のエンティティ数
    EntityID m_nextEntity = 1;             ///< 次に割り当てるEntityID

    std::unordered_map<ComponentID, uint32_t> m_componentSizes;   ///< コンポーネントIDごとのバイトサイズ
    std::unordered_map<ComponentID, std::string> m_componentNames; ///< コンポーネントIDごとのデバッグ名
    std::vector<std::unique_ptr<System>> m_systems;                ///< 登録されたシステムのリスト
};
