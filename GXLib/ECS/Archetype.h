...
private:
    ...
    std::vector<EntityID> m_entities;                            ///< このアーキタイプに属するエンティティIDリスト
    std::unordered_map<ComponentID, uint32_t> m_componentIndex;  ///< ComponentID -> ストレージインデックス
    std::vector<ComponentStorage> m_storages;                    ///< コンポーネント型ごとの連続ストレージ
};
