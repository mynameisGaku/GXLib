#pragma once
/// @file VectorSet.h
/// @brief ソート済みベクタセット — キーの動的配列に二分探索でアクセス
...
/// @brief ソート済みベクタセット (sorted vector of keys, binary search lookup)
/// @tparam Key キー型
/// @tparam Compare 比較関数
/// @tparam Alloc アロケータ
...
public:
    ...
    VectorSet() noexcept = default;
    explicit VectorSet(const Compare& comp) : m_comp(comp) {}

    VectorSet(std::initializer_list<Key> init)
    {
        Reserve(init.size());
        for (auto& k : init)
            Insert(k);
    ...
    VectorSet(const VectorSet& other) : m_comp(other.m_comp)
    {
        if (other.m_size > 0)
        {
            GrowTo(other.m_size);
    ...
    InsertResult Insert(Key&& key)
    {
        ...
        EnsureCapacity(m_size + 1);
        ShiftRight(idx);
        ::new (static_cast<void*>(m_data + idx)) Key(std::move(key));
        ++m_size;
        return { m_data + idx, true };
    }
    ...
    friend bool operator==(const VectorSet& a, const VectorSet& b) noexcept
    {
        if (a.m_size != b.m_size) return false;
        for (size_type i = 0; i < a.m_size; ++i)
            if (a.m_comp(a.m_data[i], b.m_data[i]) || a.m_comp(b.m_data[i], a.m_data[i]))
                return false;
        return true;
    }

    friend bool operator!=(const VectorSet& a, const VectorSet& b) noexcept { return !(a == b); }

    bool Validate() const noexcept
    {
        for (size_type i = 1; i < m_size; ++i)
            if (!m_comp(m_data[i - 1], m_data[i]))
                return false;
        return true;
    }
...
private:
    ...
    void ShiftLeft(size_type idx)
    {
        ...
        {
            ::new (static_cast<void*>(m_data + i - 1)) Key(std::move(m_data[i]));
            m_data[i].~Key();
        }
    }
