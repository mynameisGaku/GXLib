...
public:
    ...
    void Build(const gx::Vector<std::pair<T, AABB3D>>& objects)
    {
        m_objects = objects;
        m_nodes.clear();
        if (m_objects.empty()) return;

        gx::Vector<int> indices(m_objects.size());
    ...
    /// @param ray 検索レイ
    /// @param results 見つかったオブジェクトの出力先
    void Query(const Ray& ray, gx::Vector<T>& results) const
    {
        if (m_nodes.empty()) return;
...
private:
    ...
    int BuildRecursive(gx::Vector<int>& indices, int start, int end)
    {
    ...
    void QueryNode(int nodeIdx, const AABB3D& area, gx::Vector<T>& results) const
    {
    ...
    void QueryRayNode(int nodeIdx, const Ray& ray, gx::Vector<T>& results) const
    {
        const Node& node = m_nodes[nodeIdx];
        float t;
    ...
    gx::Vector<Node> m_nodes;
    gx::Vector<std::pair<T, AABB3D>> m_objects;
};
