...
{
    if (!handleBuf) {
        GX_LOG_ERROR("LoadDivGraph: handleBuf is null (file '%s')", filePath ? filePath : "(null)");
        return -1;
    }
    if (allNum <= 0 || allNum > 4096) {
        GX_LOG_ERROR("LoadDivGraph: allNum=%d out of range [1,4096] (file '%s')", allNum, filePath ? filePath : "(null)");
        return -1;
    }
    if (xNum <= 0 || yNum <= 0 || xSize <= 0 || ySize <= 0) {
        GX_LOG_ERROR("LoadDivGraph: invalid grid xNum=%d yNum=%d xSize=%d ySize=%d (file '%s')", xNum, yNum, xSize, ySize, filePath ? filePath : "(null)");
        return -1;
    }
    ...
    if (baseHandle < 0) {
        GX_LOG_ERROR("LoadDivGraph: failed to load base texture '%s'", filePath ? filePath : "(null)");
        return -1;
    }
...
int GetGraphSize(int handle, int* width, int* height)
{
    ...
    if (!tex) {
        GX_LOG_ERROR("GetGraphSize: invalid handle %d (texture not loaded or already deleted)", handle);
        return -1;
    }
