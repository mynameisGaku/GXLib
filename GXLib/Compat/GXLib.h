...
/// @brief 前フレームからの経過時間（デルタタイム）を取得する
/// @return 経過時間（秒）
float GetDeltaTime();
...
/// @brief ポストエフェクト適用マスクを設定する
/// @param mask 適用するエフェクトのビットマスク（gx::PostFXFlag の組合せ）
void SetPostFXMask(PostFXFlag mask);

/// @brief ポストエフェクトの一括有効/無効を切り替える
/// @param enabled true で全エフェクト有効 (PostFXFlag::All)、false で全無効 (PostFXFlag::None)
...
///
/// 画面右上にエンジン内部状態（FPS、メモリ、描画統計、PostEffect、Audio等）を
/// 半透明パネルで表示する。描画は ScreenFlip() 内部で自動的に行われる。
/// @code
/// // 例: F3キーでデバッグ表示切替
/// if (CheckHitKey(KEY_INPUT_F3)) ToggleDebugOverlay();
/// @endcode
...
int CreateFontToHandle(const char* fontName, int size, int thick, int fontType = -1);
...
/// @brief マテリアルのシェーダーモデルを設定する (ADR-0017 L2 カスタムモデル対応)
/// @param materialHandle マテリアルハンドル
/// @param shaderModelId シェーダーモデルID
///         - 0=Standard, 1=Unlit, 2=Toon, 3=Phong, 4=Subsurface, 5=ClearCoat (組み込み)
///         - 6-254 = ShaderRegistry::RegisterCustomShaderModel で登録したカスタムID
///         - 255 = Custom (Material::shaderHandle 経路、従来どおり)
/// @return 成功時 0、失敗時 -1
/// @code
/// // Rainbow カスタムシェーダを登録後、マテリアルに適用
/// SetMaterialShaderModel(matH, 100);
/// @endcode
int SetMaterialShaderModel(int materialHandle, int shaderModelId);

/// @brief ロード済みモデルの全マテリアルにシェーダーモデルを一括適用する
/// @param modelHandle LoadModel で取得したモデルハンドル
/// @param shaderModelId シェーダーモデルID (SetMaterialShaderModel 参照)
/// @return 適用に成功したマテリアル数 (≥0)、失敗時 -1
/// @code
/// int m = LoadModel("player.fbx");
/// SetModelShaderModel(m, 100);  // 全マテリアルを Rainbow に切り替え
/// @endcode
