...
struct ShaderKey
{
    gx::WString filePath;      ///< HLSLファイルパス
    gx::WString entryPoint;    ///< エントリポイント関数名
    gx::WString target;        ///< ターゲットプロファイル（vs_6_0等）
    gx::Vector<std::pair<gx::WString, gx::WString>> defines;  ///< #defineマクロ定義
...
struct ShaderKeyHasher
{
    ...
    {
        size_t h = std::hash<gx::WString>{}(key.filePath);
        h ^= std::hash<gx::WString>{}(key.entryPoint) + 0x9e3779b9 + (h << 6) + (h >> 2);
        h ^= std::hash<gx::WString>{}(key.target) + 0x9e3779b9 + (h << 6) + (h >> 2);
        for (auto& [name, value] : key.defines)
        {
            h ^= std::hash<gx::WString>{}(name) + 0x9e3779b9 + (h << 6) + (h >> 2);
            h ^= std::hash<gx::WString>{}(value) + 0x9e3779b9 + (h << 6) + (h >> 2);
        }
        return h;
    }
};
...
public:
    ...
    /// @return コンパイル済みシェーダー
    ShaderBlob GetShader(const gx::WString& filePath,
    ...
    /// @return コンパイル済みシェーダー
    ShaderBlob GetShaderVariant(const gx::WString& filePath,
                                const gx::WString& entryPoint,
                                const gx::WString& target,
    ...
    /// @return 登録ID（解除時に使用）
    PSOCallbackID RegisterPSORebuilder(const gx::WString& shaderPath, PSORebuilder callback);
    ...
    bool InvalidateFile(const gx::WString& filePath);
    ...
    /// @return エラー文字列
    const gx::String& GetLastError() const { return m_lastError; }
...
private:
    ...
    struct RebuilderEntry
    {
        ...
        gx::WString  shaderPath;   ///< 正規化済みファイルパス
