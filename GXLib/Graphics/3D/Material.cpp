...
bool MaterialManager::SetShaderModel(int handle, gxfmt::ShaderModel model)
{
    Material* mat = GetMaterial(handle);
    if (!mat) return false;
    mat->shaderModel = model;
    // シェーダーモデルのデフォルトパラメータを反映 (上書き、ユーザー値は上位APIで再設定)
    mat->shaderParams = gxfmt::DefaultShaderModelParams(model);
    return true;
}
