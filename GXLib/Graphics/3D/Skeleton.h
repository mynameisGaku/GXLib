...
struct Joint
{
    gx::String name;                ///< ジョイント名
    int         parentIndex = -1;    ///< 親ジョイントのインデックス（ルートなら -1）
    Matrix4x4   inverseBindMatrix;   ///< バインドポーズの逆行列（スキニング計算用）
    Matrix4x4   localTransform;      ///< 親ジョイント空間でのローカル変換行列
};
...
struct BoneConstants
{
    ...
    Matrix4x4 boneMatrices[k_MaxBones];             ///< 転置済みボーン行列配列
};
...
public:
    ...
    void ComputeGlobalTransforms(const Matrix4x4* localTransforms,
    ...
    void ComputeBoneMatrices(const Matrix4x4* globalTransforms,
                              Matrix4x4* boneMatrices) const;

    /// @brief 全ジョイントを取得する
    /// @return ジョイント配列への const 参照
    const gx::Vector<Joint>& GetJoints() const { return m_joints; }
    ...
    int FindJointIndex(const gx::String& name) const;

private:
    gx::Vector<Joint> m_joints;
};
