...
struct JointConstraint
{
    ...
    DirectX::XMFLOAT3 axis = { 1.0f, 0.0f, 0.0f }; ///< ヒンジ軸
};
...
struct FullBodyIKChain
{
    ...
    std::vector<int> boneIndices;                    ///< ボーンインデックス（ルート→エフェクタ順）
    DirectX::XMFLOAT3 target = { 0, 0, 0 };        ///< IKターゲット位置
    float weight = 1.0f;                             ///< FK/IKブレンドウェイト（0=FK, 1=IK）
    uint32_t maxIterations = 20;                     ///< 最大反復回数
    float tolerance = 0.001f;                        ///< 収束許容誤差（ワールド単位）
    std::vector<JointConstraint> constraints;        ///< ボーンごとの関節制約
};
...
public:
    ...
    /// @brief チェーンを設定
    void SetupChain(IKChainId id, const std::vector<int>& boneIndices);
    ...
    /// @brief ターゲット位置を設定
    void SetTarget(IKChainId id, const DirectX::XMFLOAT3& target);

    /// @brief ターゲット位置を取得
    DirectX::XMFLOAT3 GetTarget(IKChainId id) const;
    ...
    /// @brief LookAtターゲット設定
    void SetLookAtTarget(const DirectX::XMFLOAT3& target);
    ...
    /// @brief ソルブ順序を取得
    std::vector<IKChainId> GetSolveOrder() const;
...
private:
    ...
    std::unordered_map<IKChainId, FullBodyIKChain, IKChainIdHash> m_chains;  ///< IKチェーンマップ

    DirectX::XMFLOAT3 m_lookAtTarget = { 0, 0, 1 };  ///< LookAtターゲット位置
