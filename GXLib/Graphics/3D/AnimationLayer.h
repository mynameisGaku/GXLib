...
private:
    std::vector<float> m_weights;  ///< ボーンごとのウェイト値（0〜1）
};
...
struct BonePose
{
    DirectX::XMFLOAT3 position = { 0, 0, 0 };    ///< ボーンの位置
    DirectX::XMFLOAT4 rotation = { 0, 0, 0, 1 }; ///< ボーンの回転（クォータニオン）
    DirectX::XMFLOAT3 scale    = { 1, 1, 1 };     ///< ボーンのスケール
};
...
public:
    ...
    /// @brief レイヤーを追加
    uint32_t AddLayer(const std::string& name, AnimLayerBlendMode mode = AnimLayerBlendMode::Override);
...
private:
    std::vector<AnimationLayer> m_layers;  ///< レイヤー配列（評価順）
};
