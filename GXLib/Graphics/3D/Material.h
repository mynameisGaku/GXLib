...
{
    Color    albedoFactor;       ///< アルベド（基本色）RGBA
    ...
    Vector3  emissiveFactor;     ///< 自発光色 (RGB)
...
public:
    ...
    /// @brief マテリアルのシェーダーモデルを設定する (ADR-0017 L2)
    /// @param handle マテリアルハンドル
    /// @param model シェーダーモデル (組み込み 0-5 + ShaderRegistry に登録したカスタム 6-254)
    /// @return 成功した場合true
    bool SetShaderModel(int handle, gxfmt::ShaderModel model);
...
private:
    ...
    gx::Vector<MaterialEntry> m_entries;
    gx::Vector<int>           m_freeHandles;
    int                        m_nextHandle = 0;
};
