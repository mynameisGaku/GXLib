...
struct LegConfig
{
    ...
    DirectX::XMFLOAT3 footOffset = { 0, 0, 0 }; ///< 足先オフセット (ローカル)
...
struct GaitPattern
{
    ...
    std::vector<float>   legPhaseOffsets; ///< 各脚の位相オフセット (0-1, e.g. {0, 0.5, 0.25, 0.75})
...
struct LegState
{
    ...
    DirectX::XMFLOAT3 footWorldPosition   = { 0, 0, 0 }; ///< 現在の足位置 (ワールド)
    DirectX::XMFLOAT3 targetFootPosition  = { 0, 0, 0 }; ///< IKターゲット位置
...
struct LookAtConfig
{
    ...
    std::vector<int> eyeJointIndices; ///< 眼球ジョイントインデックス
...
public:
    ...
    void SetMovementDirection(const DirectX::XMFLOAT3& dir) { m_movementDir = dir; }
    ...
    void SetLookAtTarget(const DirectX::XMFLOAT3& target) { m_lookAtTarget = target; m_hasLookAt = true; }

    /// @brief LookAtターゲットを取得する
    DirectX::XMFLOAT3 GetLookAtTarget() const { return m_lookAtTarget; }
    ...
    DirectX::XMFLOAT3 GetBodyOffset() const { return m_bodyOffset; }

    /// @brief ボディの傾き (オイラー角: pitch, yaw, roll)
    DirectX::XMFLOAT3 GetBodyTilt() const { return m_bodyTilt; }

    /// @brief 指定脚のIKターゲット (ワールド座標)
    DirectX::XMFLOAT3 GetFootTarget(int legIndex) const;
...
private:
    struct LegData
    {
        LegConfig config;
        LegState  state;
        DirectX::XMFLOAT3 liftoffPos = { 0, 0, 0 };
        DirectX::XMFLOAT3 landingPos = { 0, 0, 0 };
    };

    GaitPattern     m_gait;
    MotionConfig    m_motionConfig;
    LookAtConfig    m_lookAtConfig;
    std::vector<LegData> m_legs;

    DirectX::XMFLOAT3 m_movementDir   = { 0, 0, 1 };
    float              m_movementSpeed = 0.0f;
    float              m_groundHeight  = 0.0f;
    float              m_globalPhase   = 0.0f; ///< グローバルサイクル位相 (0-1)

    DirectX::XMFLOAT3 m_lookAtTarget  = { 0, 0, 1 };
    bool               m_hasLookAt    = false;

    DirectX::XMFLOAT3 m_bodyOffset = { 0, 0, 0 };
    DirectX::XMFLOAT3 m_bodyTilt   = { 0, 0, 0 };
