...
struct InstanceData
{
    Matrix4x4 world;               ///< ワールド変換行列（転置済み）
    Matrix4x4 worldInvTranspose;   ///< 法線変換用逆転置行列（転置済み）
};
...
public:
    ...
    void AddInstance(const Matrix4x4& worldMatrix);
...
private:
    ...
    gx::Vector<InstanceData> m_instances;
