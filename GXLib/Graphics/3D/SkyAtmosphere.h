...
public:
    ...
    /// @param c RGB各波長の散乱係数
    void SetRayleighCoefficients(const Vector3& c) { m_rayleighCoeff = c; }
    ...
    /// @return RGB色
    static Vector3 ComputeSunColor(float sunElevation);
...
private:
    ...
    // 太陽
    Vector3 m_sunDirection = { 0.0f, 1.0f, 0.0f };   ///< 太陽の方向ベクトル
    Vector3 m_sunColor = { 1.0f, 0.98f, 0.95f };     ///< 太陽の色（リニアRGB）
    ...
    // Rayleigh散乱
    Vector3 m_rayleighCoeff = { 5.5e-6f, 13.0e-6f, 22.4e-6f };  ///< Rayleigh散乱係数（RGB）
