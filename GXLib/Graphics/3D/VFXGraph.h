...
public:
    ...
    /// @brief JSONからロード
    bool LoadFromJSON(const gx::String& jsonPath);

    /// @brief JSONに保存
    bool SaveToJSON(const gx::String& jsonPath) const;

    /// @brief バイナリからロード
    bool LoadFromBinary(const gx::String& path);

    /// @brief バイナリに保存
    bool SaveToBinary(const gx::String& path) const;

    /// @brief エフェクトをGPUParticleSystemに適用
    void ApplyToParticleSystem(GPUParticleSystem& ps, uint32_t emitterIndex = 0) const;

private:
    gx::String m_name;                    ///< エフェクト名
    gx::Vector<VFXEmitter> m_emitters;    ///< エミッター配列
};
...
private:
    ...
    Vector3 m_position = {0,0,0};        ///< インスタンスのワールド位置
