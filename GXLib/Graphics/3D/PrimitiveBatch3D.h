...
public:
    ...
    /// @param color 線の色
    void DrawWireBox(const Vector3& center, const Vector3& extents, const Vector4& color);
    ...
    void DrawWireSphere(const Vector3& center, float radius, const Vector4& color,
    ...
    /// @param divisions 分割数
    /// @param color 線の色
    void DrawGrid(float size, uint32_t divisions, const Vector4& color);
    ...
    void DrawWireCone(const Vector3& center, const Vector3& direction,
    ...
    void DrawWireCapsule(const Vector3& p0, const Vector3& p1, float radius,
                         const Vector4& color, uint32_t segments = 8);

    /// @brief ワイヤフレームの視錐台を描画する（逆VP行列からNDCコーナーを逆投影）
    /// @param inverseViewProjection 逆ViewProjection行列
    /// @param color 線の色
    void DrawWireFrustum(const Matrix4x4& inverseViewProjection, const Vector4& color);
    ...
    /// @param color 線の色
    /// @param segments 円の分割数（デフォルト16）
    void DrawWireCircle(const Vector3& center, const Vector3& normal, float radius,
                        const Vector4& color, uint32_t segments = 16);

    /// @brief XYZ軸を描画する（赤=X, 緑=Y, 青=Z）
    /// @param origin 軸の原点
    /// @param size 軸の長さ
    /// @param alpha 線のアルファ値（デフォルト1.0）
    void DrawAxis(const Vector3& origin, float size, float alpha = 1.0f);
