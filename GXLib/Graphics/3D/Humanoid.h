...
{
    gx::Array<int, static_cast<size_t>(HumanoidBone::Count)> joints;
...
private:
    ...
    gx::Vector<TransformTRS> m_sourceBindPose;
    gx::Vector<TransformTRS> m_targetBindPose;
    gx::Vector<float>        m_sourceBoneLength;  ///< ソース側の各ボーン長
    gx::Vector<float>        m_targetBoneLength;  ///< ターゲット側の各ボーン長
};
