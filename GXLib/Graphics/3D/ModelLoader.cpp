...
static gx::WString GetBaseDir(const gx::WString& path)
{
    ...
    if (p.has_parent_path())
        return gx::WString(p.parent_path().wstring().c_str());
    return L".";
}
...
{
    std::filesystem::path p(path.c_str());
    return ToLower(gx::WString(p.extension().wstring().c_str()));
}
                                          ...
                                          const gx::WString& fbxFilePath)
{
    ...
    if (fileName && *fileName)
    {
        ...
        if (std::filesystem::exists(absPath.c_str()))
            return absPath;

        // 2. 絶対パスが存在しない場合、ファイル名だけ抽出してFBXディレクトリで探す
        std::filesystem::path fn = std::filesystem::path(absPath.c_str()).filename();
        gx::WString localPath = (std::filesystem::path(baseDir.c_str()) / fn).wstring().c_str();
        if (std::filesystem::exists(localPath.c_str()))
            return localPath;

        // 3. よくあるサブディレクトリも試す
        for (const wchar_t* sub : { L"textures", L"Textures", L"texture", L"tex" })
        {
            gx::WString subPath = (std::filesystem::path(baseDir.c_str()) / sub / fn).wstring().c_str();
            if (std::filesystem::exists(subPath.c_str()))
                return subPath;
        }

        // 3b. FBXファイル名ベースの .fbm サブディレクトリ
        {
            gx::WString fbxStem = std::filesystem::path(fbxFilePath.c_str()).stem().wstring().c_str();
            gx::WString fbmDir = fbxStem + L".fbm";
            gx::WString fbmPath = (std::filesystem::path(baseDir.c_str()) / fbmDir.c_str() / fn).wstring().c_str();
            if (std::filesystem::exists(fbmPath.c_str()))
                return fbmPath;
        }
    }
    ...
    if (relName && *relName)
    {
        ...
        std::filesystem::path p(relPath.c_str());
        gx::WString resolved = (std::filesystem::path(baseDir.c_str()) / p).wstring().c_str();
        if (std::filesystem::exists(resolved.c_str()))
            return resolved;

        // 相対パスのファイル名だけでも試す
        gx::WString fnOnly = (std::filesystem::path(baseDir.c_str()) / p.filename()).wstring().c_str();
        if (std::filesystem::exists(fnOnly.c_str()))
            return fnOnly;
    }
    ...
    if (p.is_absolute())
        return path;
    return gx::WString((std::filesystem::path(baseDir.c_str()) / p).wstring().c_str());
}
