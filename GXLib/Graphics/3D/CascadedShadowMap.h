...
struct ShadowConstants
{
    ...
    Matrix4x4 lightVP[k_NumCascades];       ///< カスケードごとのライト空間VP行列（転置済み）
...
public:
    ...
    void Update(const Camera3D& camera, const Vector3& lightDirection);
...
private:
    ...
    gx::Array<ShadowMap, k_NumCascades>  m_shadowMaps;
    ShadowConstants                       m_constants = {};
    D3D12_GPU_DESCRIPTOR_HANDLE           m_srvGPUHandleStart = {};

    /// カスケード分割比率（nearZ~farZの割合）
    gx::Array<float, k_NumCascades> m_cascadeRatios = { 0.05f, 0.15f, 0.4f, 1.0f };
