...
{
    XMFLOAT3 position = { 0, 0, 0 };      ///< ウェイポイントの位置
    XMFLOAT3 lookTarget = { 0, 0, 0 };    ///< 注視点
...
public:
    ...
    XMFLOAT3 GetRailPosition() const;
    XMFLOAT3 GetRailLookTarget() const;
    ...
    /// @brief オービットの注視点を設定
    void SetOrbitTarget(const XMFLOAT3& target) { m_orbitTarget = target; }
    XMFLOAT3 GetOrbitTarget() const { return m_orbitTarget; }
    /// @brief マウス入力でオービット回転
...
private:
    ...
    XMFLOAT3 m_shakeOffset = { 0, 0, 0 };        ///< 現在のシェイクオフセット
    float m_shakeRotation = 0.0f;                 ///< 現在のシェイク回転量（ラジアン）
    uint32_t m_shakeSeed = 12345;                 ///< ノイズ生成用シード値

    // レール
    std::vector<CameraRailPoint> m_railPoints;    ///< レールウェイポイント配列
    ...
    XMFLOAT3 m_orbitTarget = { 0, 0, 0 };        ///< オービット注視点
    ...
    XMFLOAT3 CatmullRom(const XMFLOAT3& p0, const XMFLOAT3& p1, const XMFLOAT3& p2, const XMFLOAT3& p3, float t) const;
};
