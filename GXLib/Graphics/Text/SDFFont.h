...
public:
    ...
    /// @brief テキストの描画幅を計算する
    float MeasureWidth(const std::wstring& text, float fontSize) const;
...
private:
    ...
    std::unordered_map<uint32_t, SDFGlyph> m_glyphs;   ///< 文字コード→グリフ情報マップ
    std::vector<KerningPair> m_kernings;               ///< カーニングペア配列
};
