...
public:
    ...
    const std::vector<MipRequest>& GetMipRequests() const { return m_mipRequests; }
...
private:
    ...
    std::vector<ComPtr<ID3D12Resource>> m_feedbackMaps;   ///< フィードバックマップリソース
    std::vector<ComPtr<ID3D12Resource>> m_resolveBuffers; ///< リゾルブ先バッファ
    std::vector<MipRequest>             m_mipRequests;    ///< 分析結果のリクエストリスト
};
