...
public:
    ...
    int LoadTexture(const gx::WString& filePath);
    ...
    const gx::WString& GetFilePath(int handle) const;
    ...
    /// @brief テクスチャをファイルからインプレースリロードする (ホットリロード用)
    ///
    /// 既存ハンドルを維持したまま、テクスチャデータを差し替える。
    /// @param handle リロード対象のテクスチャハンドル
    /// @param filePath テクスチャファイルパス
    /// @return 成功時true
    bool ReloadTextureInPlace(int handle, const gx::WString& filePath);
...
private:
    ...
    struct TextureEntry
    {
        ...
        gx::WString filePath;              ///< 読み込み元のファイルパス
        bool isRegionOnly = false;          ///< trueならUV矩形のみ（テクスチャは別ハンドルが所有）
    };

    gx::Vector<TextureEntry>                  m_entries;      ///< ハンドル→エントリのマッピング
    gx::HashMap<gx::WString, int>      m_pathCache;    ///< パス→ハンドルのキャッシュ
    gx::Vector<int>                           m_freeHandles;  ///< 解放済みハンドルの再利用リスト
