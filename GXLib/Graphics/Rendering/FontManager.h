...
public:
    ...
    int CreateFont(const gx::WString& fontName, int fontSize, bool bold = false, bool italic = false);
...
private:
    ...
    struct FontEntry
    {
        ...
        gx::HashMap<wchar_t, GlyphInfo> glyphs; ///< 文字→グリフ情報のキャッシュ
        gx::Vector<uint32_t> atlasPixels;              ///< CPU側のアトラスピクセルデータ (ARGB)
    ...
    gx::Vector<FontEntry> m_entries;     ///< フォントエントリの配列
    gx::Vector<int>       m_freeHandles; ///< 解放済みハンドルのフリーリスト
    int                    m_nextHandle = 0;
};
