...
void FontManager::Shutdown()
{
    ...
    // --- COM オブジェクトを "意図的にリーク" する理由 --------------------
    // WIC/DirectWrite/Direct2D の ComPtr::Reset() はプロセス終了フェーズで
    // 稀にアクセス違反を起こすことがある (wincodecs.dll の内部状態や
    // COM アパートメント解体順に依存)。原因は FontManager 単独では
    // 制御できない (GXLib 外部の COM 使用者が状態を変えている可能性)。
    //
    // 対処: Detach() で参照を手放す (Release を呼ばない)。
    // これらはプロセス寿命を通じて使う grobal factory なので、
    // OS がプロセス終了時に自動でクリーンアップする。リークといっても
    // プロセス生存中の1インスタンス × 3ファクトリのみ、実害ゼロ。
    // 通常の Reset / destructor 経路は ~FontManager で走るが、
    // そこでも Detach 済みなので安全。
    m_wicFactory.Detach();
    m_dwriteFactory.Detach();
    m_d2dFactory.Detach();

    // CoUninitialize も呼ばない — グローバル COM 状態はプロセス終了に任せる。
    // (FontManager::Initialize で S_OK を受け取っていても、他所が同じ
    //  スレッドで CoInit 済みだった場合に teardown 競合が起きうる)
    m_comInitialized = false;

    GX_LOG_INFO("FontManager shutdown (COM objects intentionally leaked; OS will reclaim on process exit)");
}
