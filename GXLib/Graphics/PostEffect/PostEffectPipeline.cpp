                                  ...
                                  float deltaTime, PostFXFlag mask)
{
    ...
    // Layer 2: AfterSSAO カスタムエフェクト (ADR-0017 L2)
    RunCustomEffectsHDR(PostFXInsertPoint::AfterSSAO, currentHDR);
    ...
    // Layer 2: AfterBloom カスタムエフェクト (ADR-0017 L2)
    RunCustomEffectsHDR(PostFXInsertPoint::AfterBloom, currentHDR);
    ...
    // Layer 2: AfterDoF カスタムエフェクト (ADR-0017 L2)
    RunCustomEffectsHDR(PostFXInsertPoint::AfterDoF, currentHDR);
    ...
    // Layer 2: AfterMotionBlur カスタムエフェクト (ADR-0017 L2)
    RunCustomEffectsHDR(PostFXInsertPoint::AfterMotionBlur, currentHDR);
    ...
    // Layer 2: BeforeToneMap カスタムエフェクト (ADR-0017 L2 — HDR 最終段)
    RunCustomEffectsHDR(PostFXInsertPoint::BeforeToneMap, currentHDR);
    ...
    // Layer 2: LDR空間のカスタムエフェクトが登録されていれば LDR チェーンを通す (ADR-0017 L2)
    bool hasCustomLDR = false;
    for (const auto& e : m_customEffects)
    {
        if (e.point == PostFXInsertPoint::AfterToneMap ||
            e.point == PostFXInsertPoint::BeforeFXAA)
        {
            hasCustomLDR = true;
            break;
        }
    }
    bool hasLDREffects = fxaaMasked || vignetteMasked || hasCustomLDR;
    ...
    m_ldrRT[0].TransitionTo(m_cmdList, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);

    int ldrIdx = 0;

    // Layer 2: AfterToneMap カスタムエフェクト (ADR-0017 L2 — LDR 空間)
    RunCustomEffectsLDR(PostFXInsertPoint::AfterToneMap, ldrIdx);

    // Layer 2: BeforeFXAA カスタムエフェクト (ADR-0017 L2 — LDR 空間)
    RunCustomEffectsLDR(PostFXInsertPoint::BeforeFXAA, ldrIdx);
...
// =========================================================================
// Layer 2 カスタムポストエフェクト挿入 (ADR-0017 L2, T1.5)
// =========================================================================

void PostEffectPipeline::RunCustomEffectsHDR(PostFXInsertPoint point, RenderTarget*& currentHDR)
{
    for (auto& e : m_customEffects)
    {
        if (e.point != point || !e.effect) continue;
        RenderTarget* dest = (currentHDR == &m_hdrRT) ? &m_hdrPingPongRT : &m_hdrRT;
        e.effect->Execute(m_cmdList, *currentHDR, *dest, m_width, m_height);
        dest->TransitionTo(m_cmdList, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        currentHDR = dest;
    }
}

void PostEffectPipeline::RunCustomEffectsLDR(PostFXInsertPoint point, int& ldrIdx)
{
    for (auto& e : m_customEffects)
    {
        if (e.point != point || !e.effect) continue;
        int dstIdx = 1 - ldrIdx;
        e.effect->Execute(m_cmdList, m_ldrRT[ldrIdx], m_ldrRT[dstIdx], m_width, m_height);
        m_ldrRT[dstIdx].TransitionTo(m_cmdList, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        ldrIdx = dstIdx;
    }
}

void PostEffectPipeline::InsertCustomEffect(const char* name, PostFXInsertPoint point,
                                            std::unique_ptr<ICustomEffect> effect)
{
    if (!name || !effect)
    {
        GX_LOG_ERROR("InsertCustomEffect: name or effect is null");
        return;
    }

    for (auto& entry : m_customEffects)
    {
        if (entry.name == name)
        {
            entry.point  = point;
    ...
    m_customEffects.push_back({ gx::String(name), point, std::move(effect) });
}

void PostEffectPipeline::RemoveCustomEffect(const char* name)
{
    if (!name) return;
    m_customEffects.erase(
        std::remove_if(m_customEffects.begin(), m_customEffects.end(),
                       [name](const CustomEffectEntry& e) { return e.name == name; }),
        m_customEffects.end());
}
