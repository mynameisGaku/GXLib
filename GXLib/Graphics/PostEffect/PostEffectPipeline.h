...
public:
    ...
    /// @param mask 適用するエフェクトのビットマスク (デフォルト: 全エフェクト)
    ...
    const LensFlare& GetLensFlare() const { return m_lensFlare; }
    ...
    void SetMainLightDirection(const Vector3& dir) { m_mainLightDirection = dir; }
    const Vector3& GetMainLightDirection() const { return m_mainLightDirection; }

    // --- 設定ファイル ---
    bool LoadSettings(const gx::String& filePath);
    bool SaveSettings(const gx::String& filePath) const;
    ...
    // =========================================================================
    // Layer 2 拡張ポイント — カスタム PostFX 挿入 (ADR-0017 L2, T1.5)
    // =========================================================================

    /// @brief カスタムポストエフェクトのインターフェース。Layer 2 ユーザーが
    ///        エフェクトチェーンに自作処理を挿入するために継承する。
    ///
    /// Custom post-effect interface. Layer 2 users subclass this and insert it
    /// into the pipeline at a specified point. The pipeline calls Execute() once
    /// per frame with ping-pong RTs. The custom effect reads `input` and writes
    /// to `output`; the pipeline handles resource barriers.
    ///
    /// @code
    /// // Example: a desaturation effect
    /// class DesaturateEffect : public PostEffectPipeline::ICustomEffect {
    ///     void Execute(ID3D12GraphicsCommandList* cmd,
    ///                  RenderTarget& input, RenderTarget& output,
    ///                  uint32_t w, uint32_t h) override {
    ///         // Bind your PSO, set root params, dispatch/draw
    ///     }
    /// };
    /// pipeline.InsertCustomEffect("Desaturate",
    ///     PostFXInsertPoint::AfterBloom,
    ///     std::make_unique<DesaturateEffect>());
    /// @endcode
    struct ICustomEffect
    {
        virtual ~ICustomEffect() = default;

        /// @brief エフェクトを実行する。パイプラインが input→output の
        ///        リソースバリアを自動で発行する。
        /// @param cmdList  コマンドリスト（パイプラインが管理）
        /// @param input    入力 RT (SRV 状態)
        /// @param output   出力 RT (RTV 状態)
        /// @param width    RT幅 (DynamicResolution適用後の値)
        /// @param height   RT高さ
    ...
    /// @brief カスタムエフェクトの挿入位置
    enum class PostFXInsertPoint : uint32_t
    {
        AfterSSAO,        ///< SSAO後 (HDR空間)
        AfterBloom,       ///< Bloom後 (HDR空間)
        AfterDoF,         ///< DoF後 (HDR空間)
        AfterMotionBlur,  ///< MotionBlur後 (HDR空間)
        BeforeToneMap,    ///< トーンマップ直前 (HDR最終)
        AfterToneMap,     ///< トーンマップ後 (LDR空間)
        BeforeFXAA,       ///< FXAA直前 (LDR空間)
    };

    /// @brief カスタムポストエフェクトを挿入する。
    ///        同じ名前が既に存在する場合は置換する。
    /// @param name    識別名 (ログ・デバッグ表示に使用)
    /// @param point   挿入位置 (PostFXInsertPoint)
    /// @param effect  エフェクト実装 (ownership transferred)
    void InsertCustomEffect(const char* name, PostFXInsertPoint point,
                            std::unique_ptr<ICustomEffect> effect);

    /// @brief カスタムポストエフェクトを削除する。
    /// @param name InsertCustomEffect で指定した名前
    void RemoveCustomEffect(const char* name);

    /// @brief 登録されたカスタムエフェクト数を返す。
    size_t GetCustomEffectCount() const { return m_customEffects.size(); }
...
private:
    ...
    {
        gx::String name;
        PostFXInsertPoint point;
        std::unique_ptr<ICustomEffect> effect;
    };
    gx::Vector<CustomEffectEntry> m_customEffects;
    ...
    /// @brief 指定挿入点で登録済みカスタムエフェクトを順に実行する (ADR-0017 L2)
    /// @details HDR空間用: currentHDR を ping-pong しながら各カスタムエフェクトを実行。
    ///          出力は SRV 状態に遷移させて次のエフェクトに渡せるようにする。
    void RunCustomEffectsHDR(PostFXInsertPoint point, RenderTarget*& currentHDR);

    /// @brief LDR空間用カスタムエフェクト実行ヘルパー
    /// @param ldrIdx in/out: 現在の m_ldrRT インデックス (0 or 1)。swap するたび更新される
    void RunCustomEffectsLDR(PostFXInsertPoint point, int& ldrIdx);
