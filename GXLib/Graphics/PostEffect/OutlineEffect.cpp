...
bool OutlineEffect::Initialize(ID3D12Device* device, uint32_t width, uint32_t height)
{
    ...
    ShaderLibrary::Instance().RegisterPSORebuilder(
        L"Shaders/Outline.hlsl",
        [this](ID3D12Device* dev) { return CreatePipelines(dev); }
    );

    GX_LOG_INFO("OutlineEffect initialized (%dx%d)", width, height);
    return true;
}

bool OutlineEffect::CreatePipelines(ID3D12Device* device)
{
    auto vs = m_shader.CompileFromFile(L"Shaders/Outline.hlsl", L"FullscreenVS", L"vs_6_0");
    if (!vs.valid) return false;

    auto ps = m_shader.CompileFromFile(L"Shaders/Outline.hlsl", L"PSOutline", L"ps_6_0");
...
void OutlineEffect::UpdateSRVHeap(RenderTarget& srcHDR, DepthBuffer& depth, uint32_t frameIndex)
{
...
void OutlineEffect::Execute(ID3D12GraphicsCommandList* cmdList, uint32_t frameIndex,
                              ...
                              DepthBuffer& depth, const Camera3D& camera)
{
    ...
    constants.lineColor       = m_lineColor;
...
void OutlineEffect::OnResize(ID3D12Device* device, uint32_t width, uint32_t height)
{
    m_width  = width;
    m_height = height;
}
