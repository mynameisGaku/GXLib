...
struct ShaderNode
{
    ...
    gx::String name;                                ///< ノード名（表示用）
    float posX = 0.0f;                               ///< エディタ上のX座標
    float posY = 0.0f;                               ///< エディタ上のY座標
    gx::Vector<ShaderPin> pins;                     ///< ピン一覧（入力+出力）
...
{
    gx::Vector<ShaderNode> nodes;             ///< 全ノード
    gx::Vector<ShaderConnection> connections;  ///< 全接続
    gx::String name;                           ///< グラフ名
    gx::String targetModel = "PBR";            ///< ターゲットシェーダモデル
};
...
struct CompileResult
{
    ...
    gx::String hlslCode;                   ///< 生成されたHLSLコード
    gx::Vector<gx::String> errors;        ///< エラーメッセージ一覧
    gx::Vector<gx::String> warnings;      ///< 警告メッセージ一覧
};
...
public:
    ...
    /// @brief ノードを追加する
    /// @return 新しいノードの ID
    uint32_t AddNode(ShaderNodeType type, const gx::String& name = "",
    ...
    // --- ターゲットモデル ---
    void SetTargetModel(const gx::String& model) { m_targetModel = model; }
    const gx::String& GetTargetModel() const { return m_targetModel; }
    ...
    /// @brief 指定ノードのピン一覧を返す
    gx::Vector<ShaderPin> GetNodePins(uint32_t nodeId) const;

    /// @brief ノードプロパティを設定する
    void SetNodeProperty(uint32_t nodeId, const gx::String& key, const gx::String& value);

    /// @brief ノードプロパティを取得する
    gx::String GetNodeProperty(uint32_t nodeId, const gx::String& key) const;
...
private:
    ...
    /// @brief トポロジカルソートを行う
    gx::Vector<uint32_t> TopologicalSort() const;

    /// @brief ノードの HLSL 変数宣言コードを生成する
    gx::String GenerateHLSLForNode(const ShaderNode& node, uint32_t index) const;

    /// @brief ピン型名を HLSL 型文字列に変換する
    static gx::String PinTypeToHLSL(ShaderPinType type);
    ...
    gx::Vector<ShaderNode>       m_nodes;          ///< ノード一覧
    gx::Vector<ShaderConnection> m_connections;    ///< 接続一覧
    gx::String                   m_targetModel = "PBR"; ///< ターゲットシェーダモデル
