...
{
    std::vector<TimelineBar> cpuBars;       ///< CPUタイムラインバー
    std::vector<TimelineBar> gpuBars;       ///< GPUタイムラインバー
...
public:
    ...
    /// @return スナップショットの配列
    std::vector<FrameSnapshot> GetFrameHistory(uint32_t count) const;
    ...
    /// @brief セクションを選択する
    /// @param name セクション名
    void SelectSection(const std::string& name) { m_selectedSection = name; }

    /// @brief 選択中のセクション名を取得する
    /// @return セクション名（未選択の場合は空文字列）
    const std::string& GetSelectedSection() const { return m_selectedSection; }
    ...
    /// @return ARGB色
    static uint32_t GetCategoryColor(const std::string& category);

private:
    /// @brief カテゴリ名を推定する
    /// @param sectionName セクション名
    /// @return カテゴリ名
    static std::string InferCategory(const std::string& sectionName);
    ...
    std::string       m_selectedSection;                        ///< 選択中のセクション名
