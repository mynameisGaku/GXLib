...
using DialogueValue = std::variant<bool, int, float, std::string>;

/// @brief 子ノードリンク（ノード間の接続）
struct DialogueLink
{
    uint32_t targetNodeId = UINT32_MAX; ///< 遷移先ノードID
    std::string condition;  ///< 条件式 (空=無条件)
...
struct DialogueGraphNode
{
    ...
    std::string emotion;                                ///< 感情タグ（演出用）
    std::vector<DialogueLink> links;                    ///< 子ノードへのリンク
    std::string setVariable;                            ///< ノード通過時に設定する変数名
...
public:
    ...
    /// @return 割り当てられたノードID
    uint32_t AddNode(DialogueNodeType type, const std::string& speaker = "", const std::string& text = "");
    ...
    void AddLink(uint32_t fromNodeId, uint32_t toNodeId, const std::string& label = "", const std::string& condition = "");
    ...
    /// @param name 変数名
    /// @param value 設定する値
    void SetVariable(const std::string& name, const DialogueValue& value);

    /// @brief 変数の値を取得する
    /// @param name 変数名
    /// @return 変数の値（未設定の場合はデフォルト値）
    DialogueValue GetVariable(const std::string& name) const;
    ...
    const std::vector<uint32_t>& GetVisitedNodes() const { return m_visited; }
...
private:
    ...
    std::vector<DialogueGraphNode> m_nodes;                     ///< 全ノード
    ...
    std::unordered_map<std::string, DialogueValue> m_variables; ///< 変数テーブル
    std::vector<uint32_t> m_visited;                            ///< 訪問済みノード履歴
