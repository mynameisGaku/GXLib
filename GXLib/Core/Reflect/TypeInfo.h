...
public:
    ...
    TypeInfo(const std::string& name, uint32_t typeSize);
    ...
    const std::vector<PropertyMeta>& GetProperties() const { return m_properties; }

    /// @brief 名前でプロパティを検索する（見つからない場合nullptrを返す）
    /// @param name 検索するプロパティ名
    /// @return 見つかったプロパティへのポインタ。見つからない場合は nullptr
    const PropertyMeta* FindProperty(const std::string& name) const;
    ...
    /// @return プロパティの値
    bool GetBool(const void* obj, const std::string& propName) const;

    /// @brief int プロパティの値を取得する
    /// @param obj 対象オブジェクトへのポインタ
    /// @param propName プロパティ名
    /// @return プロパティの値
    int GetInt(const void* obj, const std::string& propName) const;

    /// @brief float プロパティの値を取得する
    /// @param obj 対象オブジェクトへのポインタ
    /// @param propName プロパティ名
    /// @return プロパティの値
    float GetFloat(const void* obj, const std::string& propName) const;

    /// @brief string プロパティの値を取得する
    /// @param obj 対象オブジェクトへのポインタ
    /// @param propName プロパティ名
    /// @return プロパティの値
    std::string GetString(const void* obj, const std::string& propName) const;
    ...
    /// @param propName プロパティ名
    /// @param value 設定する値
    void SetBool(void* obj, const std::string& propName, bool value) const;

    /// @brief int プロパティの値を設定する
    /// @param obj 対象オブジェクトへのポインタ
    /// @param propName プロパティ名
    /// @param value 設定する値
    void SetInt(void* obj, const std::string& propName, int value) const;

    /// @brief float プロパティの値を設定する
    /// @param obj 対象オブジェクトへのポインタ
    /// @param propName プロパティ名
    /// @param value 設定する値
    void SetFloat(void* obj, const std::string& propName, float value) const;

    /// @brief string プロパティの値を設定する
    /// @param obj 対象オブジェクトへのポインタ
    /// @param propName プロパティ名
    /// @param value 設定する値
    void SetString(void* obj, const std::string& propName, const std::string& value) const;
    ...
    /// @param baseName 基底型名
    void SetBaseType(const std::string& baseName) { m_baseName = baseName; }

    /// @brief 基底型の名前を取得する
    /// @return 基底型名（未設定の場合は空文字列）
    const std::string& GetBaseTypeName() const { return m_baseName; }

private:
    std::string m_name;                    ///< 型名
    std::string m_baseName;                ///< 基底型名（継承追跡用）
    uint32_t m_typeSize = 0;               ///< sizeof で得られるバイトサイズ
    uint32_t m_typeHash = 0;               ///< 型名の FNV-1a ハッシュ値
    std::vector<PropertyMeta> m_properties; ///< 登録済みプロパティ一覧
