...
public:
    ...
    const TypeInfo* FindType(const gx::String& name) const;
    ...
    gx::Vector<const TypeInfo*> GetAllTypes() const;
...
private:
    ...
    gx::HashMap<gx::String, std::unique_ptr<TypeInfo>> m_types; ///< 名前→TypeInfo のマップ（所有）
    gx::HashMap<uint32_t, TypeInfo*> m_hashMap;                ///< ハッシュ→TypeInfo の逆引きマップ
};
