...
struct DialogueLine
{
    std::string speaker;     ///< キャラクター名
    std::string text;        ///< 台詞テキスト
...
{
    std::string text;                ///< プレイヤーに表示される選択肢テキスト
    std::string nextSequenceId;     ///< 選択時にジャンプするシーケンス（空 = 続行）
...
{
    std::string id;                    ///< 一意の識別子
    std::vector<DialogueLine> lines;   ///< 順番に表示される行
    std::vector<DialogueChoice> choices; ///< 最後の行の後に表示される選択肢（空 = なし）
    std::string nextSequenceId;        ///< 次のシーケンスへの自動チェーン（空 = 停止）
};

/// @brief 台詞イベントコールバック
struct DialogueCallbacks
{
    std::function<void(const std::string& speaker, const std::string& text)> onLineStarted;   ///< 台詞行が表示開始した時のコールバック
    std::function<void(const std::vector<DialogueChoice>& choices)> onChoicesPresented;       ///< 選択肢が提示された時のコールバック
...
public:
    ...
    bool StartSequence(const std::string& sequenceId);
    ...
    /// @param id シーケンスID
    /// @return 登録済みならtrue
    bool HasSequence(const std::string& id) const { return m_sequences.count(id) > 0; }

private:
    /// @brief 現在の行を表示しコールバックを発火する
    void ShowCurrentLine();

    std::unordered_map<std::string, DialogueSequence> m_sequences; ///< 登録済みシーケンス
    ...
    static const std::vector<DialogueChoice> s_emptyChoices;       ///< 空の選択肢（参照返却用）
};
