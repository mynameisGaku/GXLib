...
public:
    ...
    static bool SaveToFile(const Scene& scene, const std::string& path, SceneFileFormat format = SceneFileFormat::Text);

    /// @brief ファイルからシーンを読み込む
    /// @param path 入力ファイルパス
    /// @param format ファイル形式（Text または Binary）
    /// @return 読み込まれたシーン。失敗時は nullptr
    static std::unique_ptr<Scene> LoadFromFile(const std::string& path, SceneFileFormat format = SceneFileFormat::Text);
    ...
    static std::unique_ptr<Scene> DeserializeFromString(const std::string& data);

    /// @brief シーンをバイナリ形式にシリアライズする
    /// @param scene シリアライズするシーン
    /// @return バイナリデータ
    static std::vector<uint8_t> SerializeToBinary(const Scene& scene);

    /// @brief バイナリ形式からシーンをデシリアライズする
    /// @param data バイナリデータ
    /// @return デシリアライズされたシーン。失敗時は nullptr
    static std::unique_ptr<Scene> DeserializeFromBinary(const std::vector<uint8_t>& data);
};
