...
{
    std::string componentName; ///< 対象コンポーネント名
    std::string propertyName;  ///< 対象プロパティ名
    std::string value;         ///< 上書き値（文字列表現）
...
struct ComponentOverride
{
    ...
    std::vector<PropertyOverride> properties;  ///< プロパティ上書きの一覧
};
...
struct PrefabData
{
    ...
    std::vector<uint8_t> entityData;           ///< シリアライズされたエンティティデータ
    std::vector<uint64_t> childPrefabIds;      ///< 子プレハブIDの一覧
};
...
struct PrefabVariantData
{
    ...
    std::vector<ComponentOverride> overrides; ///< コンポーネント上書きの一覧
    std::unordered_map<uint32_t, std::vector<ComponentOverride>> childOverrides; ///< 子エンティティごとの上書き
};
...
public:
    ...
    uint64_t CreateVariant(uint64_t basePrefabId, const std::string& name);
    ...
    /// @return バリアントIDの配列
    std::vector<uint64_t> GetVariantsOf(uint64_t basePrefabId) const;
    ...
    /// @param value 上書き値
    /// @return 成功した場合 true
    bool AddPropertyOverride(uint64_t variantId, const std::string& componentName,
    ...
    /// @param propertyName 対象プロパティ名
    /// @return 見つかって削除された場合 true
    bool RemovePropertyOverride(uint64_t variantId, const std::string& componentName,
    ...
    /// @return 成功した場合 true
    bool AddComponentOverride(uint64_t variantId, const std::string& componentName, bool isAdded);

    /// @brief コンポーネント上書きを削除する
    /// @param variantId バリアントID
    /// @param componentName 対象コンポーネント名
    /// @return 見つかって削除された場合 true
    bool RemoveComponentOverride(uint64_t variantId, const std::string& componentName);

    /// @brief バリアントの全上書き情報を取得する
    /// @param variantId バリアントID
    /// @return コンポーネント上書きの配列
    std::vector<ComponentOverride> GetOverrides(uint64_t variantId) const;
    ...
    bool IsOverridden(uint64_t variantId, const std::string& componentName,
    ...
    /// @param propertyName 対象プロパティ名
    /// @return 見つかって削除された場合 true
    bool RevertOverride(uint64_t variantId, const std::string& componentName,
                        const std::string& propertyName);
    ...
    /// @return 上書き適用後のデータ
    std::vector<uint8_t> ApplyOverrides(uint64_t variantId, const std::vector<uint8_t>& targetData) const;
    ...
    uint64_t CreateNestedVariant(uint64_t parentVariantId, const std::string& name);

    /// @brief ルートプレハブから指定バリアントまでの継承チェーンを取得する
    /// @param variantId バリアントID
    /// @return ルートからこのバリアントまでのIDの配列
    std::vector<uint64_t> GetInheritanceChain(uint64_t variantId) const;
...
private:
    ...
    std::unordered_map<uint64_t, PrefabData> m_prefabs;           ///< ID→プレハブデータのマップ
    std::unordered_map<uint64_t, PrefabVariantData> m_variants;   ///< ID→バリアントデータのマップ
