...
public:
    ...
    /// @return 成功でtrue
    bool LoadBaseline(const std::string& filePath);

    /// @brief 現在の結果をベースラインとして保存する
    /// @param filePath 保存先ファイルパス
    /// @return 成功でtrue
    bool SaveBaseline(const std::string& filePath) const;
...
private:
    BenchmarkResult RunSingle(const std::string& name, std::function<void()>& fn,
                               const BenchmarkConfig& config);
    ...
    std::vector<BenchmarkEntry> m_benchmarks;      ///< 登録済みベンチマーク配列
    std::vector<BenchmarkResult> m_results;         ///< 最後の実行結果配列
    std::vector<BenchmarkBaseline> m_baselines;     ///< 読み込み済みベースライン配列
};
