...
using HotReloadCallback = std::function<void(const std::wstring& filePath, HotReloadAssetType type)>;
...
/// mgr.OnReload(HotReloadAssetType::Shader, [](const std::wstring& path, HotReloadAssetType) {
///     // シェーダーを再コンパイル
/// });
/// @endcode
...
public:
    ...
    void WatchAssetDirectory(const std::wstring& dir, HotReloadAssetType type);
...
private:
    ...
    /// @param path ファイルパス
    /// @return 判定されたアセット種別
    HotReloadAssetType ClassifyFile(const std::wstring& path) const;

    /// @brief デバウンス中の変更情報
    struct PendingChange {
        std::wstring filePath;       ///< 変更されたファイルパス
    ...
    std::vector<PendingChange> m_pendingChanges;                ///< デバウンス中の変更リスト
    std::unordered_map<int, HotReloadCallback> m_callbacks;     ///< アセット種別ごとのコールバック
