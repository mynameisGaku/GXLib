...
struct MemorySnapshot
{
    ...
    std::unordered_map<std::string, uint64_t> categoryUsage; ///< カテゴリ別使用量
};
...
public:
    ...
    /// @brief アロケーションを記録する
    void RecordAllocation(void* ptr, size_t size, const std::string& category = "Default");
    ...
    /// @return 使用量（バイト）
    uint64_t GetCategoryUsage(const std::string& category) const;
    ...
    std::vector<AllocationRecord> GetLeakedAllocations() const;
...
private:
    std::unordered_map<void*, AllocationRecord> m_activeAllocations; ///< アクティブアロケーション
