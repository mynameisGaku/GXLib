...
struct ReloadEvent
{
    ...
    std::wstring filePath;          ///< リロード対象のファイルパス
...
public:
    ...
    void RegisterTexture(const std::wstring& filePath, int textureHandle);

    /// @brief マテリアルのファイルパスと名前を関連付けて登録する
    /// @param filePath マテリアルファイルパス
    /// @param materialName マテリアル名
    void RegisterMaterial(const std::wstring& filePath, const std::string& materialName);

    /// @brief スクリプトのファイルパスと名前を関連付けて登録する
    /// @param filePath スクリプトファイルパス
    /// @param scriptName スクリプト名
    void RegisterScript(const std::wstring& filePath, const std::string& scriptName);
    ...
    /// @param filePath テクスチャファイルパス
    /// @return リロード成功で true
    bool ReloadTexture(const std::wstring& filePath);

    /// @brief マテリアルを手動リロードする
    /// @param filePath マテリアルファイルパス
    /// @return リロード成功で true
    bool ReloadMaterial(const std::wstring& filePath);

    /// @brief スクリプトを手動リロードする
    /// @param filePath スクリプトファイルパス
    /// @return リロード成功で true
    bool ReloadScript(const std::wstring& filePath);
...
private:
    ...
    void OnFileChanged(const std::wstring& filePath);

    /// @brief ファイルパスからリロード対象アセット種別を判定する
    /// @param path ファイルパス
    /// @return 判定されたアセット種別
    ReloadableAssetType ClassifyAsset(const std::wstring& path) const;

    HotReloadManager* m_hotReloadMgr = nullptr;   ///< ホットリロードマネージャー（非所有）
    TextureManager* m_textureManager = nullptr;    ///< テクスチャマネージャー（非所有）

    std::unordered_map<std::wstring, int> m_textureMap;          ///< テクスチャパス→ハンドルマップ
    std::unordered_map<std::wstring, std::string> m_materialMap; ///< マテリアルパス→名前マップ
    std::unordered_map<std::wstring, std::string> m_scriptMap;   ///< スクリプトパス→名前マップ
