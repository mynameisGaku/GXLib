...
public:
    ...
    /// @brief デフラグ計画を計算
    /// @return 移動計画の配列
    std::vector<DefragMove> ComputeDefragPlan() const;
...
private:
    ...
    std::vector<std::unique_ptr<TLSFBlock>> m_blockStorage;      ///< ブロックの所有権管理
    std::unordered_map<size_t, TLSFBlock*> m_offsetToBlock;      ///< オフセット→ブロックのマップ
};
