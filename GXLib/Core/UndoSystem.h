...
public:
    ...
    ValueCommand(std::string description, T oldValue, T newValue,
...
private:
    std::string m_description;            ///< コマンドの説明
    T m_oldValue;                         ///< 変更前の値
    T m_newValue;                         ///< 変更後の値
...
private:
    std::vector<std::unique_ptr<ICommand>> m_undoStack; ///< アンドゥスタック
    std::vector<std::unique_ptr<ICommand>> m_redoStack; ///< リドゥスタック
