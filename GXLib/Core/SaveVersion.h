...
struct MigrationStep
{
    ...
    std::function<bool(gx::Vector<uint8_t>&)> migrationFn; ///< マイグレーション関数
};
...
public:
    ...
    /// @param fn マイグレーション関数
    void RegisterMigration(uint32_t from, uint32_t to, const gx::String& description,
    ...
    /// @param flags フラグ
    void WriteHeader(gx::Vector<uint8_t>& data, uint32_t version = 0, uint32_t flags = 0);

    /// @brief データからヘッダーを読み取る
    /// @param data データバッファ
    /// @param outHeader 読み取ったヘッダーの出力先
    /// @return 成功でtrue
    bool ReadHeader(const gx::Vector<uint8_t>& data, SaveFormatHeader& outHeader) const;
    ...
    /// @return 成功でtrue
    bool Migrate(gx::Vector<uint8_t>& data);
    ...
    bool VerifyChecksum(const SaveFormatHeader& header, const gx::Vector<uint8_t>& data) const;
...
private:
    ...
    gx::Vector<MigrationStep> m_migrations;      ///< 登録済みマイグレーションステップ
