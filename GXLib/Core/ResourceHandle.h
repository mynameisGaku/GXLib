...
public:
    ...
    ResourceHandle<T> Find(const std::string& name) const
    {
    ...
    ResourceHandle<T> Register(const std::string& name, std::unique_ptr<T> resource)
    {
        auto it = m_nameToIndex.find(name);
        if (it != m_nameToIndex.end()) {
            auto& entry = m_entries[it->second];
...
private:
    ...
    mutable std::vector<ResourceEntry<T>> m_entries;              ///< リソースエントリ配列
    std::unordered_map<std::string, uint32_t> m_nameToIndex;     ///< 名前→インデックスのマップ
    std::vector<uint32_t> m_freeList;                            ///< 再利用可能なスロットのリスト
};
