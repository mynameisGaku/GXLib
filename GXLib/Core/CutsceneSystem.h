...
struct CutsceneAction
{
    ...
    // 移動/回転
    XMFLOAT3 targetPosition = { 0, 0, 0 }; ///< 移動先の位置
    XMFLOAT3 targetRotation = { 0, 0, 0 };  ///< 回転先のオイラー角（度）

    // フェード
    float targetAlpha = 0.0f;  ///< フェード先のアルファ値（0.0=透明、1.0=不透明）

    // 効果音再生
    std::string soundName;     ///< 再生する効果音の名前

    // 台詞表示
    std::string dialogueText;  ///< 台詞テキスト
...
{
    std::function<void(const XMFLOAT3& pos, float progress)> onMoveTo;           ///< 移動アクション時のコールバック
    std::function<void(const XMFLOAT3& rot, float progress)> onRotateTo;         ///< 回転アクション時のコールバック
    std::function<void(float alpha)> onFadeTo;                                    ///< フェードアクション時のコールバック
    std::function<void(const std::string& sound)> onPlaySound;                    ///< 効果音再生時のコールバック
    std::function<void(const std::string& speaker, const std::string& text)> onShowDialogue; ///< 台詞表示時のコールバック
};
...
private:
    std::vector<CutsceneAction> m_actions;           ///< タイムラインに登録されたアクション群
