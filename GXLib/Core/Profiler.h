...
public:
    ...
    const std::vector<SectionResult>& GetResults() const;
...
private:
    ...
    std::vector<SectionEntry>  m_openSections;     ///< 計測中のセクションスタック
    std::vector<SectionResult> m_currentResults;    ///< 現フレームの結果（書き込み中）
    std::vector<SectionResult> m_lastResults;       ///< 前フレームの結果（読み取り用）
