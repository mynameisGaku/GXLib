...
public:
    ...
    std::vector<NavPoint> FindPath(const NavPoint& start, const NavPoint& end) const;
...
private:
    ...
    /// @return ポリゴンインデックス列
    std::vector<int> AStarOnGraph(int startTri, int endTri) const;

    /// @brief ファンネルアルゴリズムでパスを平滑化
    std::vector<NavPoint> FunnelSmooth(const std::vector<int>& triPath,
    ...
    std::vector<NavTriangle> m_triangles;   ///< 三角形配列
