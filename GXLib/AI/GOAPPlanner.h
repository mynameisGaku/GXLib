...
public:
    /// @brief 状態フラグを設定する
    void Set(const gx::String& key, bool value) { m_state[key] = value; }

    /// @brief 状態フラグを取得する (未設定はfalse)
    bool Get(const gx::String& key) const
    {
    ...
    /// @brief キーが設定されているか
    bool Has(const gx::String& key) const { return m_state.count(key) > 0; }
    ...
    /// @brief 全キーバリューを取得する
    const gx::HashMap<gx::String, bool>& GetAll() const { return m_state; }
...
private:
    gx::HashMap<gx::String, bool> m_state;
};

/// @brief GOAPアクション定義
struct GOAPAction
{
    gx::String name;                  ///< アクション名
...
struct Plan
{
    gx::Vector<const GOAPAction*> actions;  ///< アクション列 (実行順)
...
public:
    ...
    /// @brief アクションを名前で削除する
    bool RemoveAction(const gx::String& name);

    /// @brief 登録アクション数
    int GetActionCount() const { return static_cast<int>(m_actions.size()); }

    /// @brief アクションを名前で取得する
    const GOAPAction* GetAction(const gx::String& name) const;

    /// @brief アクションが登録されているか
    bool HasAction(const gx::String& name) const;
    ...
    Plan MakePlanForBestGoal(const WorldState& currentState, const gx::Vector<GOAPGoal>& goals) const;
...
private:
    ...
    gx::Vector<GOAPAction> m_actions;
