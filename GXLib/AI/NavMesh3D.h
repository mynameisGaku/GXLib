...
struct OffMeshLink
{
    ...
    std::string tag;           ///< タグ("jump", "ladder", "teleport"等)
};
...
public:
    ...
    /// @return 成功時true
    bool Build(const XMFLOAT3& minBounds, const XMFLOAT3& maxBounds, float voxelSize = 1.0f);
    ...
    bool WorldToVoxel(const XMFLOAT3& worldPos, int& outX, int& outY, int& outZ) const;

    /// @brief ボクセルインデックスをワールド座標に変換
    XMFLOAT3 VoxelToWorld(int x, int y, int z) const;
    ...
    XMFLOAT3 FindNearestOpen(const XMFLOAT3& pos) const;

    /// @brief レイキャスト（3Dボクセル空間での線分判定）
    bool Raycast(const XMFLOAT3& origin, const XMFLOAT3& direction, float maxDist,
    ...
    /// @return 最小座標
    XMFLOAT3 GetMinBounds() const { return m_minBounds; }
...
private:
    ...
    std::vector<VoxelState> m_voxels;       ///< ボクセル状態配列
    std::vector<float> m_costs;             ///< ボクセルごとのコスト倍率
    std::vector<OffMeshLink> m_offMeshLinks; ///< Off-meshリンクリスト
    XMFLOAT3 m_minBounds = {0, 0, 0};      ///< ワールド空間の最小座標
