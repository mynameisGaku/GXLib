...
'RenderLayer-SetVisible': [
  ...
  '// デバッグレイヤーの表示切替\nif (showDebug) {\n    debugLayer->SetVisible(true);   // 表示\n} else {\n    debugLayer->SetVisible(false);  // 非表示（描画はされるが合成されない）\n}',
...
'LayerStack-Sort': [
  ...
  '// Z-order変更後にソートを通知\nsceneLayer->SetZOrder(5);\nuiLayer->SetZOrder(20);\nlayerStack.MarkDirty();  // 次のGetSortedLayers()で自動ソートされる',
...
'MaskScreen-DrawRect': [
  ...
  '// 画面中央に矩形マスクを描画\nmaskScreen.Clear(cmdList, 0);  // まず全面透過にクリア\nmaskScreen.DrawFillRect(cmdList, frameIndex,\n    340.0f, 160.0f, 600.0f, 400.0f, 1.0f);  // 矩形領域を不透明に\n\n// レイヤーにマスクを適用\nsceneLayer->SetMask(maskScreen.GetAsLayer());',
  '* value=1.0fで不透明（マスク領域として表示）、0.0fで透過\n* 座標系はピクセル座標（左上原点）\n* デフォルトvalue=1.0f\n* 1回のDrawFillRectで6頂点を使用\n* x/y: 左上座標（ピクセル）、w/h: サイズ（ピクセル）'
],
...
'MaskScreen-GetSRVIndex': [
  ...
  '// マスクレイヤーを取得してシーンに適用\nGX::RenderLayer* maskLayer = maskScreen.GetAsLayer();\nsceneLayer->SetMask(maskLayer);   // マスク適用\n\n// マスク解除\nsceneLayer->SetMask(nullptr);     // マスク解除',
