...
public:
    ...
    /// @brief ネイティブXAudio2エンジンポインタを取得する（IAudioDevice実装）
    IXAudio2* GetNativeEngine() const override { return GetXAudio2(); }

    /// @brief MasteringVoiceを取得する（出力先の設定等に使う）
    /// @return MasteringVoiceポインタ。未初期化ならnullptr
    IXAudio2MasteringVoice* GetMasterVoice() const override { return m_masterVoice; }
    ...
    /// @brief 3Dオーディオが初期化済みか（IAudioDevice実装）
    bool Is3DAudioInitialized() const override { return IsX3DAudioInitialized(); }
    ...
    /// @return チャンネル数
    uint32_t GetOutputChannelCount() const override { return m_outputChannels; }
