...
{
    Camera2D cam;
    XMMATRIX vp = ToXMMATRIX(cam.GetViewProjectionMatrix(800, 600));
    Matrix4x4 m;
    XMStoreFloat4x4(XM(&m), vp);
...
TEST(Camera2DTest, VP_ZoomAffectsScale)
{
    Camera2D cam1;
    cam1.SetZoom(1.0f);
    XMMATRIX vp1 = ToXMMATRIX(cam1.GetViewProjectionMatrix(800, 600));

    Camera2D cam2;
    cam2.SetZoom(2.0f);
    XMMATRIX vp2 = ToXMMATRIX(cam2.GetViewProjectionMatrix(800, 600));

    // (100,0)の点はzoom=2で中心からより遠くに表示されるべき
    XMVECTOR pt = XMVectorSet(100.0f, 0.0f, 0.0f, 1.0f);
    Vector4 r1, r2;
    XMStoreFloat4(XM(&r1), XMVector4Transform(pt, vp1));
    XMStoreFloat4(XM(&r2), XMVector4Transform(pt, vp2));
...
TEST(Camera2DTest, VP_TranslationMovesView)
{
    ...
    Vector4 r;
    XMStoreFloat4(XM(&r), XMVector4Transform(pt, vp));
    EXPECT_NEAR(r.x, -1.0f, 0.01f);
}
...
{
    Camera2D cam;
    XMMATRIX vp1 = ToXMMATRIX(cam.GetViewProjectionMatrix(1920, 1080));
    XMMATRIX vp2 = ToXMMATRIX(cam.GetViewProjectionMatrix(800, 600));
    Matrix4x4 m1, m2;
    XMStoreFloat4x4(XM(&m1), vp1);
    XMStoreFloat4x4(XM(&m2), vp2);
